...
class PageUser extends Page
{
    ...
    public $breadcrumb = '';
    public $title = '';
    public $showContent2Box = 'mceContentBody';
    ...
    function __construct()
    {
        ...
        //Искоренить повторяющиеся слеши (///) из url - в начале URL, так как через .htaccess их убрать не удается.
        $withoutDoubleSlash = preg_replace("/\/{2,}/", "/", $_SERVER['REQUEST_URI']);
        if($_SERVER['REQUEST_URI']!==$withoutDoubleSlash){
            header("HTTP/1.1 301 Moved Permanently");
            header("Location: http://".NAME_SERVER.$withoutDoubleSlash);
            exit();
        }
        ...
        if(isset($_GET['lang_pg'])){
            $lang_pg = $_GET['lang_pg'];
        }elseif(isset($_POST['lang_pg'])){
            $lang_pg = $_POST['lang_pg']; //когда оптравляем запросы аяксом, язык передается постом.
        }
        //$lang_pg = $_GET['lang_pg'];
        // if change language, then save it new value to COOKIE and to the session
        if(isset($lang_pg)){
            setcookie('lang_pg', "", time() - 60 * 60 * 24 * 31, '/');
            setcookie('lang_pg', intval($lang_pg), time() + 60 * 60 * 24 * 30, '/');
        ...
        if(isset($_GET['lang_st'])){
            $new_lang_id = $this->Lang->GetLangCodByShortName($_GET['lang_st']);
            setcookie('lang_pg', "", time() - 60 * 60 * 24 * 31, '/');
        ...
        if(isset($lang_pg) AND !empty($lang_pg)) $_SESSION['lang_pg'] = intval($lang_pg);
        ...
        $tmp_lang = $this->Lang->GetDefFrontLangID();
        //var_dump($_SESSION);
        ...
        if (defined("_LANG_ID")) {
            ...
            if(($this->Lang->GetCountLang('front') > 1 OR isset($_GET['lang_st'])) AND _LANG_ID != $tmp_lang){
                define("_LINK", "/" . $this->Lang->GetLangShortName(_LANG_ID) . "/");
            }else{
                define("_LINK", "/");
            }
        } else {
        ...
        if(empty($this->Settings)) $this->Settings = SysSettings::GetGlobalSettings();
        //if site status Offline thne show message and go out.
        if(!$this->Settings['site_status']){
            echo $this->Settings['txt'][_LANG_ID]['site_off_message'];
            exit();
        }
        ...
        if (!isset($_SESSION['session_id'])) {
            ...
            if (!headers_sent()){
                session_start();
                $lifetime = time()+LOGOUT_USER_TIME;
                //setcookie(session_name(),session_id(),$lifetime, '/');
                //убрал '/' - корень, так как с этим параметром очень глючит сохраннеи сессии в куки в ИЕ.
                $cook = setcookie(session_name(),session_id(),$lifetime, '/');
                //echo '<br>$cook='.$cook;
            }
        }
        ...
        $this->page_encode = $this->Lang->GetDefLangEncoding($this->GetLang());

        //============ Init all objects START ============
        $this->DBPDO = DBPDO::getInstance();
        ...
        if (defined("MOD_PUBLIC") AND MOD_PUBLIC AND empty($this->Public)){
            $this->PublicLayout = check_init('PublicLayout', 'PublicLayout');
        }

        if (defined("MOD_DEALERS") AND MOD_DEALERS AND empty($this->Dealer)){
            $this->Dealer = check_init('DealerLayout', 'DealerLayout');
        }

        if (defined("MOD_INFORMER") AND MOD_INFORMER AND empty($this->informerLayout)){
            $this->informerLayout = check_init('informerLayout', 'informerLayout');
        }

        if (defined("MOD_VIP") AND MOD_VIP AND empty($this->Vip)){
            $this->VipLayout = check_init('Vip', 'Vip');
        }
...
//        var_dump($this->main_content);
        if ($this->Is_404()) {
            $content = $this->multi['MSG_404_PAGE_NOT_FOUND'];
            ...
            $contentsHtml = View::factory('/templates/tpl_main_page_content.php')
                ...
                ->bind('main_content',$this->main_content)
                ->bind('PageUser', $this);
        }else {
            $contentsHtml = View::factory('/templates/tpl_content.php')
                ...
                ->bind('breadcrumb', $this->breadcrumb)
                ->bind('content', $this->content)
                ->bind('showContent2Box',$this->showContent2Box)
                ->bind('PageUser', $this);
        }
        if (is_ajax) {
            echo $contentsHtml;
...
//        echo View::factory('/templates/tpl_main.php')
        echo View::factory('/templates/tpl_main_bootstrap.php')
            ->bind('PageUser', $this)
