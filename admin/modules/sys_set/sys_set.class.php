...
class SysSettingsAdm extends SysSettings {
    ...
    function __construct($user_id=NULL, $module=NULL, $display=NULL, $sort=NULL, $start=NULL, $width = NULL, $spr=NULL) {
        ...
        if(empty($this->SysLang)) $this->SysLang = check_init("SysLang","SysLang");
    ...
    function Show($res = '' )
    {
        ...
        $arr_tab_names[1] = $this->Msg['SYS_SET_FRONT_SETTINGS'];
        $arr_tab_names[2] = $this->Msg['SYS_SET_REDACTORS'];
        $arr_tab_names[3] = $this->Msg['SYS_SET_MAIL'];
        $arr_tab_names[4] = 'robots.txt';

        $active_tab = 1;
        $Panel->WritePanelHead( "SubPanel_", $arr_tab_names, $active_tab );
        /*========================== Tab 1 START ================================*/
        //$Panel->WritePanelHead( "SubPanel_" );
        $Panel->WriteItemHeader( $this->Msg['SYS_SET_FRONT_SETTINGS'], 1, $active_tab );
        ?>
        <table class="ContentTable" border="0" width="100%">
            <tr class="tr1">
                ...
                <td>
                    <?
                    $ln_arr = $this->SysLang->LangArray();
                    $PanelLang = new PanelLTE();
                    $PanelLang->WritePanelHead( 'site_off_message', $ln_arr, $this->lang_id);
                    while( $el = each( $ln_arr ) ){
                        $lang_id = $el['key'];
                        $lang = $el['value'];
                        $PanelLang->WriteItemHeader( $lang, $lang_id, $this->lang_id );
                        ?>
                        <table border="0" class="EditTable">
                            ...
                            <tr>
                                <td>
                                    ...
                                    $this->Form->SpecialTextArea( $this->Form->textarea_editor, 'site_off_message['.$lang_id.']', stripslashes($val), 20, 80, 'style="width:100%;"', $lang_id, 'site_off_message'.$lang_id);
                                    ?>
                                </td>
                            </tr>
                        </table>
                        <?
                        $PanelLang->WriteItemFooter();
                    }
                    $PanelLang->WritePanelFooter();
                    ?>
                </td>
            </tr>
            <tr class="tr2">
            ...
            <tr class="tr1">
                <td align="left" width="150" height="25"><?=$this->Msg['SYS_SET_CODES_AFTER_START_OF_BODY'];?>:</td>
                <?( !empty($this->Err) ? $val = $this->site_codes_body_start : $val = $mas['site_codes_body_start'] );?>
                <td align="left"><?=$this->Form->Textarea( 'site_codes_body_start', stripslashes($val), 20, 30);?></td>
            </tr>
            <tr class="tr2">
                <td align="left" width="150" height="25"><?=$this->Msg['SYS_SET_CODES_BEFORE_END_OF_BODY'];?>:</td>
                <?( !empty($this->Err) ? $val = $this->site_codes_body_end : $val = $mas['site_codes_body_end'] );?>
                <td align="left"><?=$this->Form->Textarea( 'site_codes_body_end', stripslashes($val), 20, 30);?></td>
            </tr>
        </table>
        <?
        ...
        /*========================== Tab 2 START ================================*/
        //$Panel->WritePanelHead( "SubPanel_" );
        $Panel->WriteItemHeader( $this->Msg['SYS_SET_REDACTORS'], 2, $active_tab );
        ?>
        <table class="ContentTable" border="0" width="100%">
            <tr class="tr1">
                <td align="left" width="150" height="25"><?=$this->Msg['SYS_SET_EDITER_SELECT'];?>:</td>
                <td align="left"><select name="editer">
                        <option <?if($mas['editer']=="TinyMCE4") echo "selected "?> value="TinyMCE4">TinyMCE 4 alpha</option>
                        <option <?if($mas['editer']=="TinyMCE") echo "selected "?> value="TinyMCE">TinyMCE 3</option>
        ...
        /*========================== Tab 3 START ================================*/
        $Panel->WriteItemHeader( $this->Msg['SYS_SET_MAIL'], 3, $active_tab );
        // Write Simple Panel
    ...
    */?>
            ...
            <tr class="tr1">
                <td align="left" colspan="3">
                    ...
                    $PanelLang = new PanelLTE();
                    $ln_arr = $this->SysLang->LangArray();
                    $PanelLang->WritePanelHead( 'mail_data', $ln_arr, $this->lang_id );
                    while( $el = each( $ln_arr ) ){
                        $lang_id = $el['key'];
                        $lang = $el['value'];
                        $mas_s[$lang_id] = $lang;
        ...
        /*========================== Tab 4 START ================================*/
        //$Panel->WritePanelHead( "SubPanel_" );
        $Panel->WriteItemHeader( 'robots.txt', 4, $active_tab );
        ?>
        <table class="ContentTable" border="0" width="100%">
            <tr class="tr1">
...
// ================================================================================================
    function Save()
    {
        ...
        if( $rows > 0 ){
          ...
          `site_codes_body_start`='".$this->site_codes_body_start."',
          `site_codes_body_end`='".$this->site_codes_body_end."',
