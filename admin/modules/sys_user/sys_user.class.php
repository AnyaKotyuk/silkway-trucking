...
class UserBackend  extends SysUser {
    ...
    */
    ...
    function show()
    {
        ...
        $q="SELECT `".TblSysUser."`.*, `".TblSysGroupUsers."`.name
	    FROM `".TblSysUser."` LEFT JOIN `".TblSysGroupUsers."` ON (`".TblSysUser."`.`group_id` = `".TblSysGroupUsers."`.`id`)
	    WHERE 1";
...
//        $scriplink = $scriplink.'&fltr='.$this->fltr;
          ...
          for( $i = 0; $i < $rows; ++$i )
          {
            ...
            if( $i >=$this->start && $i < ( $this->start+$this->display ) )
            {
               ...
               <tr class="<?=$class;?>">
                ...
                $id_mod_stat = 12;//$ModulesPlug->GetModuleIdByPath( 'modules/sys_stat/stat.php' );
                ?>
    ...
    function edit()
    {
        ...
        ?>
         ...
         <tr>
          <td><b><?=$this->Msg_text['_FLD_ALIAS'];?>:<span class="red">*</span></b></td>
          <td>
           <?
           if( $this->id!=NULL ) $this->Err!=NULL ? $val=$this->email : $val=$row['email'];
           else $val=$this->email;
           if( $this->id!=NULL ) $this->Form->Hidden('old_email', stripslashes($val));
           $this->Form->TextBox('email', stripslashes($val), 50);
           ?><div class="info"><?=$this->Msg_text['TXT_EMAIL_DESCR'];?></div>
          </td>
          <td>
           <div id="isalias"></div>
          </td>
         ...
         <tr>
          ...
          <td>
           ...
           $this->Form->TextBox('pass', $val, 50, 'id="pass"');?>&nbsp;<?
           $this->Form->ButtonSimple( 'pass_generate', $this->Msg_text['TXT_SITEMAP_GENERATE'], NULL, "onClick=\"passGenerate('pass')\"" );?>
          </td>
         </tr>
         <tr>
        ...
        /* set action page-adress with parameters */
         ...
         ?>
          ...
          <tr>
           ...
           <td><?$this->Form->TextBox( 'pass', '', 20, 'id="pass"');
           ?>&nbsp;<?$this->Form->ButtonSimple( 'pass_generate', $this->Msg_text['TXT_SITEMAP_GENERATE'], NULL, "onClick=\"passGenerate('pass')\"" );?>
           </td>
          </tr>
          <tr>
           <td><b><?=$this->Msg_text['_FLD_CONFIRM_PASSWORD'];?>:<span class="red">*</span></b></td>
           <td><?$this->Form->TextBox( 'confirm_pass', '', 20 );?></td>
          </tr>
          <tr>
            <td colspan="2"><table><tr><td><img src="images/icons/warning.png" alt="" title="" border="0"></td><td class="warning"><?=$this->Msg_text['_MSG_OBLIGATORY_FOR_FILLING'];?></td></tr></table></td>
          </tr>
   ...
   function CheckFields()
   {
       ...
       if( empty($this->email) ) $this->Err = $this->Err.$this->Msg_text['MSG_FLD_EMAIL_EMPTY'].'<br>';
       else{
           if( $this->old_email!=$this->email AND !$this->unique_email($this->email) ) $this->Err=$this->Err.$this->Msg->show_text('MSG_NOT_UNIQUE_ALIAS').' "'.$this->email.'" '.$this->Msg->show_text('MSG_NOT_UNIQUE_ALIAS2').'<br>';
       }
    ...
    /**
    * Class method sendNoticeEmail
    * send login and password to user email and to admin email
    * @author Igor Trokhymchuk  <ihor@seotm.com>
    * @version 1.0, 02.01.2013
    */
    function sendNoticeEmail($is_new_user)
    {
        $href = 'http://'.$_SERVER['SERVER_NAME'].'/admin/';
        if($is_new_user){
            $subject = 'Access to SEOCMS on '.$href;
        }else{
            $subject = 'New password to SEOCMS on '.$href;
        }

        if(empty($this->email)){
            $this->email = $this->GetUserEmailByUserLogin($this->login);
        }
        ...
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
            ...
            <tr>
                <td width="50">'.$this->Msg_text['FLD_LOGIN'].':</td>
                <td>'.stripslashes($this->login).'</td>
            </tr>
            <tr>
                <td>'.$this->Msg_text['FLD_PASSWORD'].':</td>
                <td>'.stripslashes($this->pass).'</td>
            </tr>
            <tr>
                <td>'.$this->Msg_text['_FLD_ALIAS'].':</td>
                <td><a href="mailto:'.stripslashes($this->email).'">'.stripslashes($this->email).'</a></td>
            </tr>
        </table>
        ';

        //================ send to user email START =========================
        $massage = $body;
        $mail_user = new Mail(_LANG_ID);

        $SysSet = new SysSettings();
	$sett = $SysSet->GetGlobalSettings();
	$mail_user->AddAddress($this->email);
        $mail_user->Subject = $subject;
        $mail_user->Body = $massage;
        $res = $mail_user->SendMail();
        //echo '<br>$res='.$res;
        //if(!$res) return false;
        //================ send to user email END =========================

        if($is_new_user){
            $subject = 'New admin user "'.stripslashes($this->login).'" on '.$href;
        }else{
            $subject = 'Change password for user "'.stripslashes($this->login).'" on '.$href;
        }
        //================ send to admins emails START =========================
        $massage = $body;
        $mail_admin = new Mail(_LANG_ID);

        $SysSet = new SysSettings();
	$sett = $SysSet->GetGlobalSettings();
	$mail_admin->AddAddress($sett['mail_admin_email']);
        $mail_admin->Subject = $subject;
        $mail_admin->Body = $massage;
        $res = $mail_admin->SendMail();
        //echo '<br>$res='.$res;
        //if(!$res) return false;
        //================ send to admins emails END =========================
        return true;
    }
...
//        echo '<br>'.$q.' $this->Rights->result='.$this->Rights->result;
    ...
    function ShowErrBackEnd()
    {
     if ($this->Err){
        ...
        <div class="err-text"><?=$this->Err;?></div>
        </fieldset>
        <?
     }
