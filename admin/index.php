...
$task = AntiHacker::AntiHackRequest('task');
...
if (!$logon->LoginCheck()) {
    if(!$ajax){
       //$pg->WriteHeader();
        ...
        echo "<script>top.window.location.href='/admin/';</script>";
    }
    //exit();
}
else {
    ...
    if( $module ) {
       ...
       //var_dump($mdl_info);
       $pg->module_name = $mdl_info['module_name'];

       $target = PageAdmin::$type_action[$mdl_info['target']];
       $mdl= '/'.$target.'/'.$mdl_info['name'];
       //echo '<br>$mdl='.$mdl.' $module='.$module.'$mdl_info[target]='.$mdl_info['target'];
       ...
       $pg->module_name = 'Dashboard';
    }

    if(!$ajax){
       $pg->WriteHeader($pg->module_name);
       $pg->WriteContentH();
       AdminHTML::PanelMainH(NULL, $pg->module_name);
    }
