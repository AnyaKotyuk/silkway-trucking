...
//            alert('k='+k);
    ...
    function WriteTopPanel( $scriptact, $par = 0 )
    {
        ...
        if( $par == 0 OR $par == 1 )
        {
            ?><button class="btn btn-primary" onclick="$('#task').val('new');$('#<?=$this->name?>').submit();"><i class="fa fa-plus"></i> <?php echo $this->msg_text['_BUTTON_NEW'];?></button> <?
        }
        if( $par == 0 OR $par == 2 )
        {
            ?><button class="btn btn-default btn-sm" onClick="if( !window.confirm('<?=$this->msg_text['_SYS_QUESTION_IS_DELETE'];?>')){return false;} else{$('#task').val('delete');$('#<?=$this->name?>').submit();}"><i class="fa fa-remove"></i> <?php echo $this->msg_text['TXT_DELETE'];?></button><?
        }
    ...
    function WriteTopPanel2( $scriptact, $newtask = NULL, $deltask = NULL )
    {
        if(!isset($this->msg_text)) $this->msg_text=check_init_txt('TblBackMulti',TblBackMulti);
        if ( !empty($newtask) ){
            ?><button class="btn btn-primary" onclick="$('#task').val('<?php echo $newtask;?>');$('#<?=$this->name?>').submit();"><i class="fa fa-plus"></i> <?php echo $this->msg_text['_BUTTON_NEW'];?></button> <?
        }
        if ( !empty($deltask) ) {
            ?><button class="btn btn-default btn-sm" onClick="if( !window.confirm('<?=$this->msg_text['_SYS_QUESTION_IS_DELETE'];?>')){return false;} else{$('#task').val('delete');$('#<?=$this->name?>').submit();}"><i class="fa fa-remove"></i> <?php echo $this->msg_text['TXT_DELETE'];?></button><?
        }
...
<img src="images/icons/save.png" alt="<?=$this->msg_text['TXT_SAVE'];?>" title="<?=$this->msg_text['TXT_SAVE'];?>" align="center" name="save" /><?=$this->msg_text['TXT_SAVE'];?></span></span>
    ...
    // Change Request Nbr:
    // ================================================================================================
    function WriteSavePanel( $scriptact, $task='save', $func=NULL ,$name = ''){
        if(!isset($this->msg_text)) $this->msg_text=check_init_txt('TblBackMulti',TblBackMulti);
        if(empty($name)) $name = $this->msg_text['TXT_SAVE'];
        ?><button class="btn btn-success" onclick="$('#task').val('<?php echo $task;?>');$('#<?=$this->name?>').submit();" <?php if( !empty($func)) {?>onClick="<?php echo $func;?>"<?}?>><i class="fa fa-save"></i> <?php echo $name;?></button> <?
    ...
    {
        if(!isset($this->msg_text)) $this->msg_text=check_init_txt('TblBackMulti',TblBackMulti);
        ?><button class="btn btn-primary" onclick="$('#task').val('<?php echo $task;?>');$('#<?=$this->name?>').submit();"><i class="glyphicon glyphicon-floppy-saved"></i> <?php echo $this->msg_text['_BUTTON_SAVE_AND_RETURN'];?></button> <?
    ...
    {
        if(!isset($this->msg_text)) $this->msg_text=check_init_txt('TblBackMulti',TblBackMulti);
        ?><button class="btn btn-default btn-sm" onclick="$('#task').val('cancel');$('#<?=$this->name?>').submit();"><i class="fa fa-reply"></i> <?php echo $this->msg_text['_BUTTON_CANCEL'];?></button> <?
    ...
    {
        if(!isset($this->msg_text)) $this->msg_text=check_init_txt('TblBackMulti',TblBackMulti);
        ?><button class="btn btn-default btn-sm" onclick="window.history.go(-1)"><i class="fa fa-reply"></i> <?php echo $this->msg_text['FLD_BACK'];?></button> <?
    ...
    {
        if(!isset($this->msg_text)) $this->msg_text=check_init_txt('TblBackMulti',TblBackMulti);
        ?><button class="btn btn-info" onClick="window.open('<?=$script?>', 'win1', 'status=no,toolbar=no,scrollbars=yes,titlebar=no,menubar=no,resizable=yes,width=640,height=480,directories=no,location=no'); return false;"><i class="fa fa-eye"></i> <?php echo $this->msg_text['_BUTTON_PREVIEW'];?></button> <?
    ...
    {
        if(!isset($this->msg_text)) $this->msg_text=check_init_txt('TblBackMulti',TblBackMulti);
        $params = "OnClick='window.open(\"".$script."\", \"\", \"width=".$width.", height=".$height.", status=0, toolbar=0, location=1, menubar=0, resizable=1, scrollbars=1\"); return false;'";
        ?><button class="btn btn-info" <?php echo $params;?>><i class="fa fa-eye"></i> <?php echo $this->msg_text['_BUTTON_PREVIEW'];?></button> <?
    ...
    {
        if(!isset($this->msg_text)) $this->msg_text=check_init_txt('TblBackMulti',TblBackMulti);
        ?><button class="btn btn-info" onClick="<?=$this->name?>.task.value='to_publish';<?=$this->name?>.submit();return false;"><i class="fa fa-upload"></i> <?php echo $this->msg_text['_BUTTON_PUBLISH'];?></button> <?
        /*?>
    ...
    {
        ?><input type="text" name="<?=$name;?>" size="<?=$size;?>" value="<?=htmlspecialchars($value);?>" class="form-control" <?=$params;?> /><?
    ...
    {
        ?><input type="submit" class="btn btn-default btn-flat" name="<?=$name;?>" value="<?=$value;?>" size="<?=$size;?>" <?=$params;?> /><?
    ...
    {
        ?><input type="button" class="btn btn-default btn-flat" name="<?=$name;?>" value="<?=$value;?>" size="<?=$size;?>" <?=$params;?> /><?
    ...
    {
        ?><textarea name="<?=$name;?>" rows="<?=$rows;?>" cols="<?=$cols;?>" class="form-control" <?if(!empty($params)) echo $params;?>><?=htmlspecialchars($value);?></textarea><?
    ...
    function SpecialTextArea( $editor=NULL, $name, $value, $rows, $cols, $params=NULL, $lang_id=NULL, $id=NULL )
    {
        ...
        switch($editor){
            case 'TinyMCE4':
                $this->TinyMCEArea4($name, $value, $rows, $cols, $params, $lang_id, $id);
                break;
            case 'TinyMCE':
        ...
        <script type="text/javascript">
            $(document).ready(function() {
                $('#<?=$id?>tinymce<?=$lang_id?>').tinymce({
                    ...
                    plugins : "pagebreak,style,contextmenu,table,advhr,advimage,advlink,inlinepopups,media,searchreplace,paste,fullscreen,noneditable,visualchars,nonbreaking,template,imagemanager,filemanager,umispoiler",
                    ...
                    theme_advanced_buttons2 : "cut,copy,paste,pastetext,pasteword,|,search,replace,|,bullist,numlist,|,outdent,indent,blockquote,|,undo,redo,|,link,unlink,anchor,image,forecolor,backcolor,|,umispoiler",
                    ...
                    startdir: "/images/imagesForTinuMCE/"
    ...
    {
        ?>
        <script type="text/javascript" src="/sys/js/tinymce4/jquery.tinymce.min.js"></script>
        <script type="text/javascript" src="/sys/js/tinymce4/tinymce.min.js"></script>
    <?
    } //end of function IncludeTinyMCE()

    // ================================================================================================
    // Function : TinyMCEArea4()
    // Date : 11.05.2015
    // Description : Write TinyMCEA Area
    // Programmer : Ihor Trokhymchuk
    // ================================================================================================
    function TinyMCEArea4( $name = '', $value = '', $rows = 4, $cols = 70, $params=NULL, $lang_id=1, $id=NULL )
    {
        $id = $id.'tinymce'.$lang_id;
        $width = $cols*10.2;
        $height = $rows*10.2;
        ?>
        <!-- Load TinyMCE -->
        <script type="text/javascript">
            $(document).ready(function() {
                tinymce.init({
                    ...
                    language:"<?if( strtolower(_LANG_SHORT)=="ua") echo "uk"; else echo strtolower(_LANG_SHORT);?>",
                    plugins: [
                        "moxiecut advlist autolink link image lists charmap print preview hr anchor pagebreak spellchecker",
                        "searchreplace wordcount visualblocks visualchars code fullscreen insertdatetime media nonbreaking",
                        "save table contextmenu directionality emoticons template paste textcolor"
                    ],
                    content_css: "css/content.css",
                    toolbar1: 'insertfile undo redo | styleselect | bold italic | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link image ',
                    toolbar2: 'print preview media | table | hr removeformat | subscript superscript | forecolor backcolor emoticons | code',
                    image_advtab: true,
                    relative_urls: false,
                    templates: [
        ...
        <!-- /TinyMCE -->
        <textarea id="<?=$id;?>" name="<?=$name;?>" rows="<?=$rows?>" cols="<?=$cols?>" <?=$params?> class="tinymce"><?=$value?></textarea>
    <?
    } //end of function TinyMCEArea4()
    ...
    /**
     * Class method Select2()
     * Write standart Select Box
     * @return void
    ...
    {
        ?>
        <select name="<?=$name;?>" <?if( !empty($width)) {?>width="<?=$width;?>"<?}?> <?if( !strstr($params, "class")){?>class="form-control"<?}?> <?=$params;?>>
            <?
            while( $el=each( $arr ) )
            {
                //echo '<br>key='.$el['key'].' $disabled_value='.$disabled_value;
                ...
                if (count($arr)==1){
                    if($el['key']) echo '<option value="'.$el['key'].'" selected '.$disabled.'>'.$el['value'].'</option>';
                    else  echo '<option value="'.$el['key'].'" '.$disabled.'>'.$el['value'].'</option>';
                }else{
                    if( $el['key']==$value   ) echo '<option value="'.$el['key'].'" selected '.$disabled.'>'.$el['value'].'</option>';
                    else echo '<option value="'.$el['key'].'" '.$disabled.'>'.$el['value'].'</option>';
                }
            }
            ?>
    ...
    /**
     * Class method Select2()
     * Write Select Box using select2 javascript plugin
     * @return void
     * @author Ihor Trokhymchuk  <ihor@seotm.com>
     */
    function Select2( $arr, $name = '', $value = NULL, $width = NULL, $params = NULL, $disabled_value = NULL )
    {
        ?>
        <select name="<?=$name;?>" <?if( !empty($width)) {?>width="<?=$width;?>"<?}?> <?if( !strstr($params, "class")){?>class="form-control select2 select2-hidden-accessible"<?}?> <?=$params;?>>
            <?
            while( $el=each( $arr ) )
            {
                if($disabled_value!=NULL AND $el['key']==$disabled_value){
    ...
    //correct for polls
    function SelectPolls( $arr, $name = '', $value = NULL, $width = NULL, $params = NULL )
    {
        ?>
        <select name="<?=$name;?>" <?if( !empty($width)) {?>width="<?=$width;?>"<?}?> <?if( !strstr($params, "class")){?>class="select2 select2-hidden-accessible"<?}?> <?=$params;?>>
            <?
            while( $el=each( $arr ) )
            {
                if (count($arr)==2){
...
<div class="err_text">'.$Err.'</div>
    ...
    function GetRequestTxtData($str=NULL, $use_strip_tags=0)
    {
        ...
        if($use_strip_tags==1){
            ...
            //$str = strip_tags(trim($str), '<iframe>,<a>,<abbr>,<acronym>,<address>,<applet>,<area>,<b>,<base>,<basefont>,<bdo>,<big>,<blockquote>,<br>,<caption>,<center>,<cite>,<code>,<col>,<colgroup>,<dd>,<del>,<dfn>,<dir>,<div>,<dl>,<dt>,<em>,<embed>,<font>,<h1>,<h2>,<h3>,<h4>,<h5>,<h6>,<head>,<hr>,<i>,<img>,<ins>,<isindex>,<kbd>,<label>,<legend>,<li>,<link>,<map>,<menu>,<meta>,<noframes>,<noscript>,<object>,<ol>,<optgroup>,<option>,<p>,<param>,<pre>,<q>,<s>,<samp>,<select>,<small>,<span>,<strike>,<strong>,<style>,<sub>,<sup>,<table>,<tbody>,<td>,<tfoot>,<th>,<thead>,<title>,<tr>,<tt>,<u>,<ul>,<script>,<i>');
            $str = trim($str);
        }
