 ...
 class SystemCurrencies {
    ...
    */
    ...
    function Converting($from_currency, $to_currency, $summa, $float = 2)
    {
        ...
        //echo '<br>$from_currency='.$from_currency.' $to_currency='.$to_currency.' $summa='.$summa.'<br>';
        if($from_currency!=$to_currency AND $from_currency!=0){
            $val_def_currency = $this->GetValue($from_currency);
            $val_id_currency = $this->GetValue($to_currency);
            //echo '<br>$val_def_currency='.$val_def_currency.' $val_id_currency='.$val_id_currency.'<br>';
            if($val_def_currency!=0){
                $ret = $val_def_currency*$val_id_currency*$summa;
            }
            /*
            if($val_def_currency!=0){
     ...
     // ================================================================================================
     // Function : ShowPriceWithConverting
     // Date : 21.01.2011
     // Parms :  $price  - price
     ...
     // ================================================================================================
     function ShowPriceWithConverting($price, $showText = true, $digits=2, $from_currency = NULL, $to_currency = NULL)
     {
         if(empty($to_currency)) $currData = $this->def_curr;
         else $currData = $this->currItem[$to_currency];
         $price = $this->Converting($from_currency, $currData['id'], $price, $digits);
         return $this->ShowPrice($price, $showText, $digits, $currData['id']);
