...
abstract class ImageK {
    ...
    static function getResizedImg($img = NULL, $size = NULL, $quality = NULL, $wtm = NULL)
    {
        ...
        $fileSize = @filesize(SITE_PATH .$img_with_path);
        $fileImgSize = @getimagesize(SITE_PATH .$img_with_path);
        //echo '<br>file_exists('.SITE_PATH .$img_with_path.')='.file_exists(SITE_PATH .$img_with_path).' $fileSize='.$fileSize.' $fileImgSize='.$fileImgSize;
        if (!file_exists(SITE_PATH .$img_with_path) || empty($fileSize) || empty($fileImgSize)){
        ...
        $arrPermitsExp = array('jpg','png','gif', 'jpeg');
        if(!in_array(strtolower($exp),$arrPermitsExp)){
            $img = DEFAULT_NO_IMAGE_PATH;
            $img_with_path = $img;
            $img_name = substr($img, strrpos($img, '/') + 1, strlen($img) - strrpos($img, '/'));
...
//        echo '<br/>$img_full_path ='.$img_full_path.' size='.$fileSize.' exist='.file_exists($img_full_path);
        //if exist local small version of the image then use it
        if(file_exists($img_full_path) && !empty($fileSize)){
            //echo 'exist';
            $str = $img_name_new;
            $flag_to_save = false;
        }
...
//                echo '<br>$settings_img_path='.$settings_img_path.' $img_full_path='.$img_full_path;
                ...
                $cnt=count($mas_img_name);
                if($cnt>2) {
                    for ($i = 0; $i < $cnt - 1; $i++) {
                        if ($i > 0) $img_name_new .= ".";
                        $img_name_new .= $mas_img_name[$i];
                    }
                }
                if (!empty($size_width)){
                    if($cnt>2) $img_name_new.= ADDITIONAL_FILES_TEXT . 'width_' . $size_width . '.' . $mas_img_name[$cnt-1];
                    else $img_name_new = $mas_img_name[0] . ADDITIONAL_FILES_TEXT . 'width_' . $size_width . '.' . $mas_img_name[1];
                }elseif (!empty($size_auto)){
                    if($cnt>2) $img_name_new.= ADDITIONAL_FILES_TEXT . 'auto_' . $size_auto . '.' . $mas_img_name[$cnt-1];
                    else $img_name_new = $mas_img_name[0] . ADDITIONAL_FILES_TEXT . 'auto_' . $size_auto . '.' . $mas_img_name[1];
                }elseif (!empty($size_height)){
                    if($cnt>2) $img_name_new.= ADDITIONAL_FILES_TEXT . 'height_' . $size_height . '.' . $mas_img_name[$cnt-1];
                    else $img_name_new = $mas_img_name[0] . ADDITIONAL_FILES_TEXT . 'height_' . $size_height . '.' . $mas_img_name[1];
                }elseif (!empty($size_square)){
                    if($cnt>2) $img_name_new.= ADDITIONAL_FILES_TEXT . 'square_' . $size_square . '.' . $mas_img_name[$cnt-1];
                    else $img_name_new = $mas_img_name[0] . ADDITIONAL_FILES_TEXT . 'square_' . $size_square . '.' . $mas_img_name[1];
                }elseif (!empty($size_rect)){
                    if($cnt>2) $img_name_new.= ADDITIONAL_FILES_TEXT . 'rect_' . $size_rect . '.' . $mas_img_name[$cnt-1];
                    else $img_name_new = $mas_img_name[0] . ADDITIONAL_FILES_TEXT . 'rect_' . $size_rect . '.' . $mas_img_name[1];
                }
