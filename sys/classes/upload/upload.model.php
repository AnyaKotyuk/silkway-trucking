...
// ================================================================================================
    function UploadModel(){
        if(!isset(UploadModel::$db))
            UploadModel::$db = DBs::getInstance();
    }
 // ================================================================================================
...
// ================================================================================================
    ...
    {
        $q = "CREATE TABLE IF NOT EXISTS `$table1` (
...
//         echo '<br>$_FILES["image"]["name"][$i]='.$_FILES["image"]["name"][$i];
           ...
           if ( isset($_FILES["image".$formId]) && is_uploaded_file($_FILES["image".$formId]["tmp_name"][$i]) && $_FILES["image".$formId]["size"][$i] ){
            ...
            if (!in_array($ext, $valid_types)) {
             ...
             if($save_file_names) $uploaddir2 =  $sys_crypt->GetTranslitStr($file_array[0]).time().$i.'.'.$ext;
             else $uploaddir2 = time().$i.'.'.$ext;
             $uploaddir = $uploaddir1."/".$uploaddir2;
//             echo '<br>$filename='.$filename.'<br> $uploaddir='.$uploaddir.'<br> $uploaddir1='.$uploaddir1.'<br> $uploaddir2='.$uploaddir2;
             //if (@move_uploaded_file($filename, $uploaddir)) {
...
//            echo '<br>$_FILES["file"]='.$_FILES["file"].' $_FILES["file"]["tmp_name"]["'.$i.'"]='.$_FILES["file"]["tmp_name"]["$i"].' $_FILES["file"]["size"]["'.$i.'"]='.$_FILES["file"]["size"]["$i"];
           if ( isset($_FILES["video"]) && is_uploaded_file($_FILES["video"]["tmp_name"][$i]) && $_FILES["video"]["size"][$i] ){
             ...
             $filenameNoExt = substr($_FILES['video']['name'][$i], 0, strrpos($_FILES['video']['name'][$i], "."));
             $ext = substr($_FILES['video']['name'][$i], 1 + strrpos($_FILES['video']['name'][$i], "."));
//            echo '<br>filesize($filename)='.filesize($filename).' $max_image_size='.$max_image_size;
            if (filesize($filename) > $max_image_size) {
            ...
            if (!in_array($ext, $valid_types)) {
             ...
             if($save_file_names) $uploaddir2 =  $sys_crypt->GetTranslitStr($filenameNoExt).'.'.$ext;
             else $uploaddir2 = time().$i.'.'.$ext;
             $uploaddir = $uploaddir1."/".$uploaddir2;
//             echo '<br>$filename='.$filename.'<br> $uploaddir='.$uploaddir.'<br> $uploaddir1='.$uploaddir1.'<br> $uploaddir2='.$uploaddir2;
             //if (@move_uploaded_file($filename, $uploaddir)) {
...
//        echo '<br>$q='.$q.' $res='.$res;
        ...
        for($i=0; $i<$rows;$i++)
        {
           ...
           {
               $result[$id]['path'] = ImageK::getResizedImg('/'.$path.'/'.$position.'/'.$row['path'],'size_auto=150', 85, NULL);//$this->GetImagePath('/'.$path.'/'.$position.'/'.$row['path'],'size_auto=150',$add_t,85);
    ...
    $size = NULL,$height=NULL,$hor_align=true,$ver_align=true,$quality = 85, $wtm = NULL, $width2 = NULL, $height2 = NULL )
    {
        ...
        for($i=0; $i<$rows;$i++)
        {
           ...
           else
           {
               ...
               elseif($ex_size==2)
                   $result[$id]['path'] = $this->GetPathImageExSizeAuto('/'.$path.'/'.$position.'/'.$row['path'],$size,$height,$add_t,$hor_align,$ver_align,$quality,$wtm);
               if(isset($width2)) {
                    if(!$ex_size)
                    ...
                    elseif($ex_size==2)
                        $result[$id]['path2'] = $this->GetPathImageExSizeAuto('/'.$path.'/'.$position.'/'.$row['path'],$width2,$height2,$add_t,$hor_align,$ver_align,$quality,$wtm);
               }
...
// ================================================================================================
    ...
    {
        $img_src = ImageK::getResizedImg($img, 'size_rect='.$width.'x'.$height, $quality, $wtm);
        return $img_src;
    } // end of function GetPathImageExSize()


    /**
     * UploadModel::GetPathImageExSizeAuto()
     * Get picture of exact size by path and show it autoscaled
     * @copyright Yaroslav Gyryn 24.04.2012
     * @param mixed $img - path of the picture
     * @param mixed $width
     * @param mixed $height
     * @param mixed $add_text - text, which used in thumbs names
     * @param bool $hor_align
     * @param bool $ver_align
     * @param integer $quality
     * @param mixed $wtm
     * @return $img_src
     */
    function GetPathImageExSizeAuto($img = NULL, $width = NULL,$height=NULL,$add_text=NULL,$hor_align=true,$ver_align=true,$quality = 85, $wtm = NULL)
    {
        ...
        //else use original image on the server SITE_PATH and make small version on local server
            ...
            $src_x = $thumb->img['x_thumb'];
            $src_y = $thumb->img['y_thumb'];
            if($src_y>$height) $thumb->size_height($height);
            elseif($src_x>$width) $thumb->size_width($width);
            else $thumb->sizeEx($width,$height);
            //echo '<br>$thumb->img[x_thumb]='.$thumb->img['x_thumb'].' $thumb->img[y_thumb]='.$thumb->img['y_thumb'];
...
//                echo '<br>$uploaddir='.$uploaddir;
     ...
     return $img_src;
    } // end of function GetPathImageExSizeAuto()
