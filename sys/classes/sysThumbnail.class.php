...
class Thumbnail {
    ...
    *@return boolean
    **/
	function Thumbnail($imgfile=NULL) 	{
    	if( !isset($imgfile) OR empty($imgfile)) return true;
    ...
    **/
	function save($save="")	{
        ...
        if (empty($save)) {
         ...
         $mask = imagecreatefrompng(SITE_PATH."/admin/images/l-t.png");  
         imagetruecolortopalette($this->img["des"],true,250);
         imagecopy ($this->img["des"],$mask,0,0,0,0,5,5);
         imagecopy ($this->img["des"],imagecreatefrompng(SITE_PATH."/admin/images/l-b.png"),0,$this->img["y"]-5,0,0,5,5);
         imagecopy ($this->img["des"],imagecreatefrompng(SITE_PATH."/admin/images/r-b.png"),$this->img["x"]-5,$this->img["y"]-5,0,0,5,5);
         imagecopy ($this->img["des"],imagecreatefrompng(SITE_PATH."/admin/images/r-t.png"),$this->img["x"]-5,0,0,0,5,5);

$trans = imagecolorat($this->img["des"],0,0);
imagecolortransparent($this->img["des"],$trans);
imagePNG($this->img["des"],"$save");
        /*if ($this->output_format=="PNG") { //PNG
            imagePNG($this->img["des"],"$save");
        } else {
...
//                }
    ...
    function processEx ($v=true,$h=true) {
...
//        echo '<br/>sdsds='.$X_des.$Y_des;
        ...
        //if (false) {
                ...
                $y_=0;
                $pa=0;
//                echo '<br/>$this->img[x]'.$this->img['x'];
//                echo '<br/>$this->img[y]'.$this->img['y'];
//                echo '<br/>$X_des'.$X_des;
//                echo '<br/>$Y_des'.$Y_des;
                if (($this->img['x']/$X_des)>($this->img['y']/$Y_des)) 
                {
                    $pa=$this->img['y']/$Y_des;
                    if($h) $x_=($this->img['x']-$pa*$X_des)/2;
                    else $y_=0;
                }
                else
                {
                    $pa=$this->img['x']/$X_des;
                    if($v) $y_=($this->img['y']-$pa*$Y_des)/2;
                    else $y_=0;
                }
//                echo '<br/>$pa'.$pa;
//                echo '<br/>$x_'.$x_;
//                echo '<br/>$y_'.$y_;
                    imagecopyresampled ($this->img["des"], $this->img["src"], 0, 0, $x_, $y_, $X_des, $Y_des, $pa*$X_des, $pa*$Y_des);
    ...
    **/
    function unique_filename ( $archive_dir , $filename , $file_prefix='') {
        ...
        **/
        ...
        $width = imagesx($img);
        $height = imagesy($img);
        //Добавлено 11.01.2009 (Нужно для получения прозрачности)
