...
* @version 1.2, 07.11.2013
* @copyright (c) 2005+ by SEOTM
*/
class Rights extends DB  {
    ...
    */
    Function __construct( $user=NULL, $module=NULL ){
        ...
        if(empty($this->user)){
            ...
            $this->user =  $logon->user_id;
        }
        $this->LoadUserAccessRights($this->user);
    }
    ...
    */
    function LoadUserAccessRights($user = NULL){
        ...
        $q = "SELECT
              ...
              `".TblSysUser."`,
              `".TblSysAccess."`
              WHERE `".TblSysUser."`.`id`='".$user."'
              AND `".TblSysAccess."`.`group`=`".TblSysUser."`.`group_id`
             ";
         $res = $this->db_Query( $q );
         //echo '<br>$q='.$q.' $res='.$res;
         if( !$res ) return false;
