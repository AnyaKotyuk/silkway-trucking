...
class DynamicPages extends Page {
    ...
    public $treeSpecContent = NULL; //array treeSpecContent[id_page]=array with spec content page
    ...
    */
    ...
    {
        $cod = 0;
        foreach($this->treePageData as $val){
            if($val['main_page']==1) $cod = $val['id'];
        }
        return $cod;
...
//        echo '<br>$q='.$q.' $res='.$res.' $db->result='.$db->result;
    ...
    function QuickSearch($search_keywords)
    {
        ...
        $str_like = $this->build_str_like_for_full(TblModPagesTxt.'.pname', $search_keywords);
        $str_like .= $filter_cr.$this->build_str_like_for_full(TblModPagesTxt.'.content', $search_keywords);

        $q = "
        SELECT `" . TblModPages . "`.id,
        `" . TblModPages . "`.id_categ,
        `" . TblModPages . "`.visible,
        `" . TblModPages . "`.level,
        `" . TblModPages . "`.move,
        `" . TblModPagesTxt . "`.*,
        (
            MATCH(`" . TblModPagesTxt . "`.`pname`) AGAINST('*".$search_keywords."*' IN BOOLEAN MODE) * 10 +
            MATCH(`" . TblModPagesTxt . "`.`content`) AGAINST('*".$search_keywords."*' IN BOOLEAN MODE)
        ) AS `relev`
            FROM `" . TblModPages . "`, `" . TblModPagesTxt . "`
            WHERE
            ($str_like )
            ...
            ORDER BY `relev` DESC ,`" . TblModPages . "`.level, `" . TblModPages . "`.move";
        $res = $this->db->db_Query($q);
        //echo '<br>q='.$q.' res='.$res;
    ...
    /**
     * Class method loadSpecContent
     * Load spec content from mod_pages_spec_content
     * @return array with spec content
     * @author Bogdan Iglinsky  <bi@seotm.com>
     * @version 1.0, 15.05.2013
     */
    function loadSpecContent(){
        if( is_array($this->treeSpecContent) ) return true;

        $q = "SELECT `".TblModPagesSpecContent."`.*
        FROM `".TblModPagesSpecContent."`
              WHERE  `".TblModPagesSpecContent."`.lang_id='".$this->lang_id."'
              ORDER BY `cod` asc";
        ...
        for($i = 0; $i < $rows; $i++){
            $row = $this->db->db_FetchAssoc($res);
            $this->treeSpecContent[$row['cod']] = $row;
        }
        return $this->treeSpecContent;
    }

    /**
     * Class method getSpecContentByCod
     * Get spec content from mass treeSpecContent by cod item
     * @param integer $cod - cod of the pages
     * @param boolean $trip_tags - kill tags or no
     * @return content of item
     * @author Bogdan Iglinsky  <bi@seotm.com>
     * @version 2.0, 15.05.2013
     */
    function getSpecContentByCod($cod = NULL,$trip_tags = '<img>'){

        if(empty($cod) || !isset($this->treeSpecContent[$cod])
            || !isset($this->treeSpecContent[$cod]['descr']) || empty($this->treeSpecContent[$cod]['descr'])) return false;
        $return = stripcslashes($this->treeSpecContent[$cod]['descr']);
        if($trip_tags!==false){
            $return = strip_tags($return,$trip_tags);
        }
        return $return;
    }

    /**
     * Class method getNameSpecContentByCod
     * Get name spec content from mass treeSpecContent by cod item
     * @param integer $cod - cod of the pages
     * @return name of item
     * @author Bogdan Iglinsky  <bi@seotm.com>
     * @version 1.0, 11.07.2014
     */
    function getNameSpecContentByCod($cod = NULL){

        if(empty($cod) || !isset($this->treeSpecContent[$cod])
            || !isset($this->treeSpecContent[$cod]['name']) || empty($this->treeSpecContent[$cod]['name'])) return false;
        $return = stripcslashes($this->treeSpecContent[$cod]['name']);
        return $return;
    }
