...
/**
 * pages_backend.class.php
 * class for Contol of Dynamic Front-end Pages
...
class PagesBackend extends DynamicPages
{
    ...
    function __construct( $user_id=NULL, $module=NULL )
    {
        ...
        $this->UploadImages  = check_init('UploadImage', 'UploadImage', '90, null, "uploads/images/pages", "mod_pages_uploads_img"');
        $this->UploadFile    = check_init('UploadClass', 'UploadClass', '90, null, "uploads/files/pages", "mod_pages_uploads_files"');
        //$this->UploadVideo = check_init('UploadVideo', 'UploadVideo', '90, null, "uploads/video/pages", "mod_pages_uploads_video"');
    ...
    function show()
    {
        ...
        ?>
        <div class="row">
            <div class="levels_tree col-lg-3 col-md-3 col-sm-3"">
                <div class="box-header with-border">
                    <div class="box-title">
                        <a href="<?=$_SERVER['PHP_SELF'].'?module='.$this->module.'&display='.$this->display.'&start='.$this->start.'&sort='.$this->sort.'&fltr='.$this->fltr;?>" title="<?=$this->multi['TXT_ROOT_CATEGORY'];?>"><img src="images/icons/categ.png" border="0" alt="<?=$this->multi['TXT_PAGES_STRUCTURE'];?>" title="<?=$this->multi['TXT_PAGES_STRUCTURE'];?>" /><?=$this->multi['TXT_PAGES_STRUCTURE'];?></a>
                    </div>
                    <div class="box-body">
    ...
    function showHTML($rows=NULL)
    {
        ...
        <td class="THead" width="10">
            ...
            <br/><a href="<?=$script2?>&sort=id" class="aTHead"><?=$this->multi['FLD_ID']?></a>
        </td>
    ...
    for( $i = 0; $i < $rows; $i++ )
    {
        ...
        <tr class="<?//=$class;?>">
            <td>
                <?php $this->Form->CheckBox( "id_del[]", $row['id'], $this->sel, "check".$i);?>
                <br/><?php echo $this->Form->Link( $this->script."&task=edit&id=".$row['id'], stripslashes( $row['id'] ), $this->multi['TXT_EDIT'] );?>
            </td>
            <td align="left" nowrap="nowrap" style="padding:5px;">
                <div><?$this->Form->Link( $this->script."&task=edit&id=".$row['id'], $pname, $this->multi['TXT_EDIT'] );?></div>
            ...
            <?if( $this->is_image==1 ){?>
                <td align="center" style="padding:2px;">
                    ...
                    if( !empty($img) ) {
                        ...
                        echo $this->Spr->ShowImageByPath($img_with_path_small, 'size_auto=75', 85, NULL, NULL);
                    }
                    ?>
                </td>
            <?}?>
    ...
    // Programmer : Igor Trokhymchuk
    // ================================================================================================
    function ShowPath($id, $path=NULL, $make_link = false)
    {
        $res = NULL;
        $devider = '→';
        if($id>0){
            $row = $this->GetTreePageData($id);
            $name = stripslashes($row['pname']);
            $link = $this->script.'&level='.$id;
            ...
            else $path = '<a href="'.$this->script.'&level=0">'.$this->multi['TXT_ROOT_CATEGORY'].'</a> '.$devider.' <span>'.$path."</span>";
        }
        else{
            $path = '<a href="'.$this->script.'&level=0">'.$this->multi['TXT_ROOT_CATEGORY'].'</a> '.$devider.' ';
        }
        return $path;
    ...
    function edit()
    {
        ...
        $Panel = check_init('PanelLTE', 'PanelLTE');
        ...
        $this->showKeyNavigation($script,$publish,$PageLayout);
        ...
        <tr>
            <td>
                ...
                $this->Form->CheckBox('visible', 'visible', $visible, 'visibleId', NULL, $this->multi['_TXT_SHOW_PAGE']);
                ?>
                ...
                $this->Form->CheckBox('publish', 'publish', $publish, 'publishId', NULL, $this->multi['_TXT_PUBLISH_PAGE']);
                ?>
        ...
        <tr valign="top">
            <td colspan="3">
                ...
                //echo '<br />$pname='.$pname;
                ?>
                <div id="urlname" class="input-group">
                    <?$this->ShowURLName($PageLayout, $ctrlscript, $pname);?>
                </div>
            </td>
        </tr>
        <tr valign="top">
            <td>
                ...
                echo '<label for="ctrlscriptId">'.$this->multi['FLD_CTRL_SCRIPT'].'</label> <small class="text-muted">('.$this->multi['HELP_FLD_CTRL_SCRIPT'].')</small>';
                ?>
        ...
        <tr>
            <td colspan="3">
                <?
                reset($this->ln_arr);
                $Panel->WritePanelHead( "SubPanel_", $this->ln_arr, $this->lang_id );
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="descr<?php echo $lang_id;?>"><?php echo $this->multi['_FLD_NAME'];?>:
                                    <div class="help"><?php echo $this->multi['_HELP_MSG_PAGE_NAME'];?></div>
                                </label>
                                ...
                                $this->Form->TextBox( 'descr['.$lang_id.']', stripslashes($val), 110, 'id="descr'.$lang_id.'"' );
                                ?>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <?php
                                if( $this->id!=NULL ) $this->Err!=NULL ? $val=$this->h1[$lang_id] : $val=$mas2['h1'];
                                else $val=$this->h1[$lang_id];
                                $this->Form->TextBox( 'h1['.$lang_id.']', stripslashes($val), 110, 'id="h1'.$lang_id.'"' );
                                ?>
                            </div>
                            ...
                            if ( $this->is_short_descr==1 ) {
                                ?>
                                <div class="form-group">
                                    <label for="short<?php echo $lang_id;?>"><?php echo $this->multi['FLD_SHORT_DESCR'];?>
                                        :
                                        <div class="help"><?php echo $this->multi['_HELP_MSG_SHORT_DESCR'];?></div>
                                    </label>

                                    <?php
                                    if ($this->id != NULL) $this->Err != NULL ? $val = $this->short[$lang_id] : $val = $mas2['short'];
                                    else $val = $this->short[$lang_id];
                                    $this->Form->Textarea('short[' . $lang_id . ']', stripslashes($val), 6, 110, 'id="short' . $lang_id . '"');
                                    ?>
                                </div>
                            <?
                            }
                        ...
                        <div class="col-md-6">
                            ...
                            ?>
                            <div class="form-group">
                                <label for="title<?php echo $lang_id;?>"><?php echo $this->multi['FLD_TITLE'];?>:
                                ...
                                $this->Form->TextBox( 'title['.$lang_id.']', stripslashes($val), 110, 'id="title'.$lang_id.'"' );
                                ?>
                            </div>

                            <div class="form-group">
                                <label for="description<?php echo $lang_id;?>"><?php echo $this->multi['FLD_DECRIPTION'];?>:
                                ...
                                $this->Form->TextArea( 'description['.$lang_id.']', stripslashes($val), 2, 110, 'id="description'.$lang_id.'"' )
                                ?>
                            </div>

                            <div class="form-group">
                                <label for="keywords<?php echo $lang_id;?>"><?php echo $this->multi['FLD_KEYWORDS'];?>:
                                    <div class="help"><?php echo $this->multi['_HELP_MSG_PAGE_KEYWORDS'];?></div>
                                </label>

                                <?php
                                if( $this->id!=NULL ) $this->Err!=NULL ? $val=$this->keywords[$lang_id] : $val=$mas2['mkeywords'];
                                else $val=$this->keywords[$lang_id];
                                $this->Form->TextArea( 'keywords['.$lang_id.']', stripslashes($val), 2, 110, 'id="keywords'.$lang_id.'"' )
                                ?>
                            </div>
                            ...
                            <table border="0" cellpadding="0" cellspacing="5">
                                <tr>
                                    <td><?
                                        ...
                                        echo $this->Spr->ShowImageByPath($img_with_path_small, 'size_width=150', 85, NULL, NULL);
                                        ?>
                                    </td>
                                    <td class='EditTable'>
                                        <br><?=$img_with_path;?><br>
    ...
    // ================================================================================================
    function showKeyNavigation($script,$publish,$PageLayout){
        ?>
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
            <tr>
                <td>
                    ...
                    if( !empty($this->id) ){
                        if( $publish==0) {
                            $this->Form->WritePreviewPanelNewWindow( $PageLayout->Link($this->id).'&preview=true' );
                        }
                    }
    ...
    function ShowURLName($PageLayout, $ctrlscript, $pname)
    {
        ...
        if( $ctrlscript){
            if ($this->level>0) $path = $PageLayout->Link($this->level);
                ...
                if(!strstr($_SERVER['SERVER_NAME'], 'www')){
                    $path = 'http://www.'.$_SERVER['SERVER_NAME'].'/';
                }else{
        ...
        ?><span class="input-group-addon text-muted"><?php echo $path;?></span><?php $this->Form->TextBox( 'name', stripslashes($val), 40, 'id="name"' );
    ...
    function save()
    {
        ...
        for($i=0;$i<$cnt;$i++){
            $lang_id = $keys[$i];
            $name = $this->Form->GetRequestTxtData($this->descr[ $lang_id ], 0);
            $h1 = $this->Form->GetRequestTxtData($this->h1[ $lang_id ], 0);
    ...
    function CheckFields()
    {
        ...
        $this->name = $this->getUniqueNameByThisLevel($this->name,$this->level,$this->id);
    ...
    // ================================================================================================
    // Function : getUniqueNameByThisLevel()
    // Date : 11.03.2014
    // Returns :      link / Void
    // Description :  Constructed a unique url page
    // Programmer :  Bogdan Iglinsky
    // ================================================================================================
    function getUniqueNameByThisLevel($name,$level,$id,$cnt = 0){
        $q = "SELECT * FROM `".TblModPages."` WHERE `name`='".$name."' AND `level`='".$level."' AND `id` != '".$id."' ";
        $res =$this->db->db_Query( $q );
//        echo '<br>$q='.$q.' $res='.$res.' $this->db->result='.$this->db->result;
        if( !$res OR !$this->db->result ) return false;
        $rows = $this->db->db_GetNumRows($res);
        if($rows==0) return $name;
        elseif($cnt<5) return $this->getUniqueNameByThisLevel($name.$id,$level,$id,$cnt+1);
        elseif($cnt==6){
            $this->Err .= $this->multi['MSG_FLD_FILENAME_ALREADY_EXIST1'].' "'.$this->name.'" '.$this->multi['MSG_FLD_FILENAME_ALREADY_EXIST2'].'<br>';
        }else $this->getUniqueNameByThisLevel($name.time(),$level,$id,6);
    ...
    function ShowErrBackEnd()
    {
        if ($this->Err){
            ...
            <div class="alert alert-danger alert-dismissible">
                ...
                <h4><i class="icon fa fa-ban"></i> <?php echo $this->multi['MSG_ERRORS'];?></h4>
                <?php echo $this->Err;?>
            </div>
        <?
        }
