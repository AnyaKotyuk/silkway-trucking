...
if(!isset($_REQUEST['dump'])) $CatalogImpExp->dump=NULL;
else $CatalogImpExp->dump=$_REQUEST['dump'];

if(!isset($_REQUEST['dumper'])) $CatalogImpExp->dumper=NULL;
else $CatalogImpExp->dumper=$_REQUEST['dumper'];

if(!isset($_REQUEST['myfiles'])) $CatalogImpExp->myfiles=NULL;
else $CatalogImpExp->myfiles=$_REQUEST['myfiles'];
switch($CatalogImpExp->task)
{
    ...
    case 'import_csv':     // Создание и обновление категорий, товаров из .csv-файла из программы E-Trade PriceList Importer
         ...
         if (in_array($ext, $filesExt)) {
          ...
          break;
    case 'import_images':
        $CatalogImpExp->import_images();
        $CatalogImpExp->repairImagesMove();
        break;
    ...
    case 'setbackup':
        ...
        if (in_array($ext, $filesExt)) {
            ...
            if ( !copy( $_FILES["filename"]["tmp_name"], $uploaddir) ) {
                ...
                $old_max_exec_time = get_cfg_var("max_execution_time");
                ini_set("max_execution_time","9999");
                $CatalogImpExp->SetCatalogBackup($uploaddir);
                ini_set("max_execution_time",$old_max_exec_time);
                $CatalogImpExp->Form();
                unlink($uploaddir);
            }
            ...
            echo 'Неверный тип файла: '.$filename.'<br/> Необходимо выбрать файл формата .sql !<br>';
            $CatalogImpExp->Form();
            return false;
        }
        break;

    case 'setbackupfromserver':
        $filename = $_SERVER['DOCUMENT_ROOT'].'/backup/'.$CatalogImpExp->myfiles;
        $CatalogImpExp->SetCatalogBackup($filename);
        $CatalogImpExp->Form();
        break;
}
?>