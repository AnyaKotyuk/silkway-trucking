...
//        echo '<br> $q='.$q.' $res='.$res.' $this->db->result='.$this->db->result;
    ...
    function GetParamsPropPrice($IdOfProps){
        $q="SELECT `".TblModCatalogProp."`.`price`,
         `".TblSysCurrencies."`.`value`,
         `".TblSysCurrencies."`.`is_default`
            FROM `".TblModCatalogProp."`,`".TblSysCurrencies."`
            WHERE `".TblModCatalogProp."`.`id` in (".$IdOfProps.")
            AND `".TblModCatalogProp."`.`visible` = '2'
            AND `".TblModCatalogProp."`.`price`!='0'
            AND `".TblModCatalogProp."`.`price_currency`=`".TblSysCurrencies."`.`id`
           ";
//        AND `".TblModCatalogProp."`.`exist` = '1'
        $res = $this->db->db_Query( $q );
//        echo '<br> $q='.$q.' $res='.$res.' $this->db->result='.$this->db->result;
...
//                echo '<br>$strText='.$strText;
        ...
        // Вывод ссылки "Все"
//        var_dump($this->id_manufac);
//        echo '<br>$this->to='.$this->to;
//        echo '$this->arr_current_img_params_value='.$this->arr_current_img_params_value;
