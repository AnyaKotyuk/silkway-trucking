...
class Catalog_content extends Catalog {
   ...
   function __construct($user_id=NULL, $module=NULL, $display=NULL, $sort=NULL, $start=NULL, $width = NULL) {
        ...
        if (empty($this->Form)) $this->Form = check_init('FormCatalogProp', 'Form', "'form_mod_catalog'");
   ...
   function GetContent($limit='limit')
   {
        ...
        if ( isset($this->settings['name']) AND $this->settings['name']=='1' ) {
            ...
            $str_like .= " OR LOWER(REPLACE(REPLACE(`" . TblModCatalogPropSprName . "`.name, ' ', ''), '-', '')) LIKE '%" . $search_keywords_no_space . "%'";
        }
        ...
        $q = "SELECT
             ...
             ";
        if ( isset($this->settings['name']) AND $this->settings['name']=='1' ) {
            $q .= ", `" . TblModCatalogPropSprName . "`.`name`";
            if( $this->srch ){
        ...
        if ( isset($this->settings['short_descr']) AND $this->settings['short_descr']=='1' ) {
            $q .= ", `" . TblModCatalogPropSprShort . "`.`name` AS `short_descr`";
        }
        if ( isset($this->settings['full_descr']) AND $this->settings['full_descr']=='1' ) {
            $q .= ", `" . TblModCatalogPropSprFull . "`.`name` AS `full_descr`";
        }
        $q .= " FROM `" . TblModCatalogProp . "`
              ...
              ";
        if ( isset($this->settings['short_descr']) AND $this->settings['short_descr']=='1' ) {
            $q .= " LEFT JOIN `" . TblModCatalogPropSprShort . "` ON (`" . TblModCatalogProp . "`.`id`=`" . TblModCatalogPropSprShort . "`.`cod` AND `" . TblModCatalogPropSprShort . "`.lang_id='" . $this->lang_id . "')";
        }
        if ( isset($this->settings['full_descr']) AND $this->settings['full_descr']=='1' ) {
            $q .= " LEFT JOIN `" . TblModCatalogPropSprFull . "` ON (`" . TblModCatalogProp . "`.`id`=`" . TblModCatalogPropSprFull . "`.`cod` AND `" . TblModCatalogPropSprFull . "`.lang_id='" . $this->lang_id . "')";
        }
        if ( isset($this->settings['name']) AND $this->settings['name']=='1' ) {
            $q .= ", `" . TblModCatalogPropSprName . "`";
        }
        $q .= ",`" . TblModCatalogSprName . "`, `" . TblModCatalog . "`, `" . TblModCatalogTranslit . "`
        ...
        $q .= " AND `" . TblModCatalogProp . "`.id_cat=`" . TblModCatalogSprName . "`.cod
      ...
      <tr>
       <td class="levels_tree">
           <div class="box-header with-border">
               <div class="box-title">
                   <a href="<?=$parent_script;?>" title="<?=$this->multi['TXT_ROOT_CATEGORY'];?>"><img src="images/icons/categ.png" border="0" alt="<?=$this->multi['TXT_CATALOG_STRUCTURE'];?>" title="<?=$this->multi['TXT_CATALOG_STRUCTURE'];?>" /><?=$this->multi['TXT_CATALOG_STRUCTURE'];?></a>
               </div>
               <div class="box-body">
   ...
   ?>
   <table border="0" cellpadding="0" cellspacing="1" class="table table-striped table-hover table-bordered">
    <tr>
    ...
    <Th class="THead"><?=$this->Form->LinkSort( $this->sort, 'move', $script2, $this->asc_desc, $this->multi['FLD_DISPLAY'], $txtSortData);?></Th>
       <?if ( isset($this->settings['info_prop']) AND $this->settings['info_prop']=='1' ) {?>
...
//        echo '<br />$q='.$q.' $this->Right->result='.$this->Right->result;
   ...
   function DelRelatPropByIdProp( $id_prop )
   {
    ...
    {
        $Panel =check_init('PanelLTE', 'PanelLTE'); //new Panel();
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="form-group">
                    <label for="id_prop" class="col-sm-1 col-md-1 col-lg-1 control-label"><?php echo $this->multi['FLD_ID'];?>:
                    </label>
                    <div class="col-sm-11">
                        <?
                        ...
                        $this->Form->TextBox( 'id', $val, 10, 'id="id_prop"' );
                        ?>
                    </div>
                </div>
                ...
                if ( isset($this->settings['cod_pli']) AND $this->settings['cod_pli']=='1' ) {
                    ?>
                    <div class="form-group">
                        <label for="cod_pli" class="col-sm-1 col-md-1 col-lg-1 control-label"><?php echo $this->multi['FLD_COD_PLI'];?>:
                        </label>
                        <div class="col-sm-11">
                            <?
                            if( $this->id!=NULL ) $this->Err!=NULL ? $cod_pli=$this->cod_pli : $cod_pli=$mas['cod_pli'];
                            else $this->Err!=NULL ? $cod_pli=$this->cod_pli : $cod_pli='';
                            $this->Form->TextBox('cod_pli', $cod_pli, 10, 'id="cod_pli"');
                            ?>
                        </div>
                    </div>
                    <?php
    ...
    <tr>
     ...
     <td>
        ...
        //print_r($arr_categs);
        $this->Form->Select2( $arr_categs, 'new_id_cat', 'categ='.$id_cat, NULL, 'style="width:320px;"' );
    ...
    <tr><td><b><?=$this->multi['FLD_MANUFAC']?>:</b>
        ...
        //print_r($arr_categs);
        $this->Form->Select2( $arr_categs, 'id_manufac', $id_manufac);
    }
   ...
   if ( isset($this->settings['share']) AND $this->settings['share']=='1' ) { ?>
      ...
      <?
        ...
        while( $el = each( $ln_arr ) )
        {
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="form-group">
                        <label for="translit<?php echo $lang_id;?>"><?php echo $this->multi['FLD_PAGE_URL'];?>:
                            <div class="help"><?php echo $this->multi['HELP_FLD_PAGE_URL'];?></div>
                        </label>
                        <div id="urlname" class="input-group margin" >
                            <span class="input-group-addon text-muted">../<?=$this->GetTranslitById( $id_cat, NULL, $lang_id );?>/</span>
                            <?
                            if( $this->id!=NULL ) {
                            ...
                            $this->Form->TextBox( 'translit['.$lang_id.']', stripslashes($val), 50, 'id="translit'.$lang_id.'"; '.$params );?>
                            <?
                            $this->Form->Hidden( 'translit_old['.$lang_id.']', stripslashes($val) );

                            if( $this->id ){
                                ?><span class="input-group-btn"><?$this->Form->ButtonSimple("btn", $this->Msg->show_text('TXT_EDIT'), NULL, "id='button".$lang_id."' onClick=\"EditTranslit('translit".$lang_id."','button".$lang_id."');\"");?></span><?
                            }
                            ?>
                        </div>
                    </div>
                    ...
                    if ( isset($this->settings['name']) AND $this->settings['name']=='1' ) {
                        ?>
                        <div class="form-group">
                            <label for="name<?php echo $lang_id;?>"><?php echo $this->multi['FLD_NAME'];?>:
                            </label>
                            <?php
                            ...
                            $this->Form->TextBox( 'name['.$lang_id.']', stripslashes($val), 80, 'id="name'.$lang_id.'"' );
                            ?>
                        </div>
                        <?
                    }
                    ...
                    <div class="form-group">
                        <label for="h1<?php echo $lang_id;?>"><?php echo $this->multi['_FLD_H1'];?>:
                            <div class="help"><?php echo $this->multi['_HELP_MSG_H1'];?></div>
                        </label>
                        <?php
                        ...
                        $this->Form->TextBox( 'h1['.$lang_id.']', stripslashes($val), 80, 'id="h1'.$lang_id.'"' );
                        ?>
                    </div>
                    ...
                    if ( isset($this->settings['short_descr']) AND $this->settings['short_descr']=='1' ) {
                        ?>
                        <div class="form-group">
                            <label for="short_descrtinymce<?php echo $lang_id;?>"><?php echo $this->multi['FLD_SHORT_DESCR'];?>:
                            </label>
                            <?php
                    ...
                    if ( isset($this->settings['full_descr']) AND $this->settings['full_descr']=='1' ) {
                        ?>
                        <div class="form-group">
                            <label for="full_descrtinymce<?php echo $lang_id;?>"><?php echo $this->multi['FLD_FULL_DESCR'];?>:
                            </label>
                            <?php
                    ...
                    if ( isset($this->settings['specif']) AND $this->settings['specif']=='1' ) {
                        ?>
                        <div class="form-group">
                            <label for="speciftinymce<?php echo $lang_id;?>"><?php echo $this->multi['FLD_SPECIF'];?>:
                            </label>
                            <?php
                    ...
                    if ( isset($this->settings['reviews']) AND $this->settings['reviews']=='1' ) {
                        ?>
                        <div class="form-group">
                            <label for="reviewstinymce<?php echo $lang_id;?>"><?php echo $this->multi['FLD_REVIEWS'];?>:
                            </label>
                            <?php
                    ...
                    if ( isset($this->settings['support']) AND $this->settings['support']=='1' ) {
                        ?>
                        <div class="form-group">
                            <label for="supporttinymce<?php echo $lang_id;?>"><?php echo $this->multi['FLD_SUPPORT'];?>:
                            </label>
                            <?php
                    ...
                    ?>
                    <div class="form-group">
                        <label for="mtitle<?php echo $lang_id;?>"><?php echo $this->multi['FLD_PAGES_TITLE'];?>:
                            <div class="help"><?php echo $this->multi['HELP_MSG_PAGE_TITLE'];?></div>
                        </label>
                        <?php
                        ...
                        $this->Form->TextBox( 'mtitle['.$lang_id.']', stripslashes($val), 70, 'id="mtitle'.$lang_id.'"' );
                        ?>
                    </div>

                    <div class="form-group">
                        <label for="mdescr<?php echo $lang_id;?>"><?php echo $this->multi['FLD_PAGES_DESCR'];?>:
                            <div class="help"><?php echo $this->multi['HELP_MSG_PAGE_DESCRIPTION'];?></div>
                        </label>
                        <?php
                        ...
                        $this->Form->TextArea( 'mdescr['.$lang_id.']', stripslashes($val), 3, 70, 'id="mdescr'.$lang_id.'"' );
                        ?>
                    </div>

                    <div class="form-group">
                        <label for="mkeywords<?php echo $lang_id;?>"><?php echo $this->multi['FLD_KEYWORDS'];?>:
                            <div class="help"><?php echo $this->multi['HELP_MSG_PAGE_KEYWORDS'];?></div>
                        </label>
                        <?php
                        ...
                        $this->Form->TextArea( 'mkeywords['.$lang_id.']', stripslashes($val),3, 70, 'id="mkeywords'.$lang_id.'"' );
                        ?>
                    </div>


                    <div class="form-group">
                        <img src='images/icons/info.png' alt='' title='' border='0' /> <?=$this->multi['HELP_MSG_META_TAGS_POSITION']?>
                    </div>
                    <?php
                    echo "</fieldset>";
                    ?>
    ...
    if ( isset($this->settings['price']) AND $this->settings['price']=='1' ) {
        ...
        <tr>
            <td colspan="2">
                <div class="form-group">
                    <label for="price" class="col-sm-2 col-md-2 col-lg-2 control-label"><?php echo $this->multi['FLD_PRICE'];?>:
                    </label>
                    <?
                    if ( isset($this->settings['price_currency']) AND $this->settings['price_currency']=='1' ){
                    ...
                    <div class="col-sm-10">
                        <div class="input-group my-group">
                            ...
                            $this->Form->TextBox( 'price', stripslashes($price), 10, 'id="price"' );
                            if($show_currencies){
                                $this->Form->Select($this->Currencies->listShortNames, 'price_currency', $price_currency);
                            }
                            ?>
                        </div>
                    </div>
    ...
    if ( isset($this->settings['opt_price']) AND $this->settings['opt_price']=='1' ) { ?>
        <tr>
            <td colspan="2">
                <div class="form-group">
                    <label for="opt_price" class="col-sm-2 col-md-2 col-lg-2 control-label"><?php echo $this->multi['FLD_OPT_PRICE'];?>:
                    </label>
                    <?
                    ...
                    <div class="col-sm-10">
                        <div class="input-group my-group">
                            ...
                            $this->Form->TextBox( 'opt_price', stripslashes($opt_price), 10, 'id="opt_price"' );
                            if($show_currencies){
                                $this->Form->Select($this->Currencies->listShortNames, 'opt_price_currency', $opt_price_currency);
                            }
                            ?>
                        </div>
                    </div>
    ...
    if ( isset($this->settings['price_levels']) AND $this->settings['price_levels']=='1' ) { ?>
        <tr>
            <td colspan="2">
                <div class="form-group">
                    <label for="" class="col-sm-2 col-md-2 col-lg-2 control-label"><?php echo $this->multi['FLD_PRICE_LEVELS'];?>:
                    </label>
                    <div class="col-sm-7">
                        <div name="price_level_data<?=$this->cnt_div;?>" id="price_level_data<?=$this->cnt_div;?>">
                            <?$this->AddHTMLPriceLevel();?>
                        </div>
                    </div>
                </div>
                <script language="JavaScript">
    ...
    if ( isset($this->settings['grnt']) AND $this->settings['grnt']=='1' ) { ?>
        <tr>
            <td colspan="2">
                <div class="form-group">
                    <label for="grnt" class="col-sm-2 col-md-2 col-lg-2 control-label"><?php echo $this->multi['FLD_GUARANTEE'];?>:
                    </label>
                    <div class="col-sm-3">
                        <?php
                        if( $this->id!=NULL ) $this->Err!=NULL ? $grnt=$this->grnt : $grnt=$mas['grnt'];
                        else $grnt=$this->grnt;
                        $this->Form->TextBox( 'grnt', stripslashes($grnt), 10, 'id="grnt"' );
                        ?>
                    </div>
                </div>
            </td>
    ...
    if ( isset($this->settings['dt']) AND $this->settings['dt']=='1' ) { ?>
        <tr>
            <td colspan="2">
                <div class="form-group">
                    <label for="dateField" class="col-sm-2 col-md-2 col-lg-2 control-label"><?php echo $this->multi['FLD_DATE_CREATE'];?>:
                    </label>
                    <div class="col-sm-3">
                        <?php
                        if( $this->id!=NULL ) {
                        ...
                        <div class="input-group">
                            ...
                            <?
                            $str = 'data-inputmask="' . "'alias': 'yyyy-mm-dd'" . ' data-mask=""';
                            $this->Form->TextBox( 'dt', $dt, 10, 'id="dateField"'.$str );
                            ?>
                        </div>
                    </div>
                </div>
    ...
    if ( isset($this->settings['cat_params']) AND $this->settings['cat_params']=='1' ) {
        ...
        $style1 = 'TR1';
        $style2 = 'TR2';
//          var_dump($id_cat_for_params);
        $params_row = $this->GetParams( $id_cat_for_params );
        $value=$this->GetParamsValuesOfProp( $this->id );
//          var_dump($value);
        $count_param_row = count($params_row);
        for ($i=0;$i<$count_param_row;$i++){
   ...
   function AddHTMLPriceLevel()
   {
        ...
        if( $this->Err!=NULL ){
            for($i=0;$i<count($this->price_level);$i++){
                ...
                ?>
                <div class="input-group my-group">
                    <span class="input-group-addon"><?php echo $this->Msg->show_text('FLD_QUANTITY').' '.$this->Msg->show_text('FLD_QUANTITY_FROM');?></span>
                    <?php $this->Form->TextBox( 'qnt_from[]', stripslashes($qnt_from), 5, 'style="width:100%;"' );?>
                    <span class="input-group-addon"><?php echo ' '.$this->Msg->show_text('FLD_QUANTITY_TO');?></span>
                    <?php $this->Form->TextBox( 'qnt_to[]', stripslashes($qnt_to), 5, 'style="width:100%;"' );?>
                    <span class="input-group-addon"><?php echo ' '.$this->Msg->show_text('FLD_PRICE');?></span>
                    <?php $this->Form->TextBox( 'price_level[]', stripslashes($price_level), 10 );
                    if($Currencies){
                        $this->Form->Select($Currencies->GetShortNamesInArray('back'), 'price_levels_currency[]', $price_levels_currency);
                    }
                    if( isset($arr_pice[$i]['id'])) {
                        ?><span class="input-group-btn"><input type="button" name="button" class="btn btn-info btn-flat" value="   -   " onclick="javascript:del_price_level('module=<?=$this->module;?>&id_price_level=<?=$arr_pice[$i]['id']?>&id=<?=$this->id;?>', 'price_level_data<?=$this->cnt_div;?>');"><?
                    }
                    ?>
                </div>
                <?
            ...
            if( $this->task!='add_html_price_level' AND is_array($arr_pice) AND count($arr_pice)>0){
                for($i=0;$i<count($arr_pice);$i++){
                    ...
                    <div class="input-group my-group">
                        ...
                        <span class="input-group-btn">
                            <button" name="button" class="btn btn-danger btn-flat" onclick="javascript:del_price_level('module=<?=$this->module;?>&id_price_level=<?=$arr_pice[$i]['id']?>&id=<?=$this->id;?>', 'price_level_data<?=$this->cnt_div;?>');">
                ...
                <div class="input-group my-group">
                    ...
                    ?>
                    <span class="input-group-btn">
                        <button" name="button" class="btn btn-default btn-flat disabled">
                            <i class="fa fa-remove"></i>
                        </button>
                    </span>
                </div>
        ...
        <div name="price_level_data<?=$this->cnt_div;?>" id="price_level_data<?=$this->cnt_div;?>">
            <?
            if( $this->task!='del_html_price_level'){
                ?><input type="button" name="button" value="   +   " onclick="javascript:add_price_level('module=<?=$this->module;?>&cnt_div=<?=$this->cnt_div;?>', 'price_level_data<?=$this->cnt_div;?>');"><?
            }
            ?>
        </div>
        <?
...
//          echo '<br>q='.$q.' res='.$res.' $this->Right->result='.$this->Right->result;
       ...
       if ( empty($this->id) ){
          ...
          if ( isset($this->settings['cod_pli']) AND $this->settings['cod_pli']=='1' AND empty($this->cod_pli) ) {
              $q="UPDATE `".TblModCatalogProp."` SET
                  `cod_pli`='".$this->id."'
                   WHERE `id`='".$this->id."'
                  ";
         ...
         <tr>
          <td>
           ...
           for($i=0;$i<count($id_del);$i++){
               ...
               $this->Form->Hidden( 'move', $this->move[$id_del[$i]] );
               if ( isset($this->settings['name']) AND $this->settings['name']=='1' ) {
            ...
            for ($i=0; $i<$rows; $i++) {
                ...
                <?//=AdminHTML::PanelSimpleH();?>
                    ...
                    <TD align=left valign="middle">
                        <a href="<?=$this->settings['img_path'].'/'.$row['id_prop'].'/'.$row['path']?>" target=_blank><?=$this->ShowCurrentImage($row['id'], 'size_auto=200', 85, NULL, "border=0");?></a>
                    </TD>
                    <TD valign="top">
