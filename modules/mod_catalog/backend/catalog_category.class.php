...
// ================================================================================================
 class Catalog_category extends Catalog {
   ...
   function Catalog_category ($user_id=NULL, $module=NULL, $display=NULL, $sort=NULL, $start=NULL, $width = NULL) {
        ...
        if (empty($this->Form)) $this->Form = check_init('FormCatalogCategories', 'Form', '"form_mod_catalog"');
   ...
   // Programmer : Igor Trokhymchuk
   // ================================================================================================
   function GetContent($limit='limit',$level_show = 'all'){
    ...
    */
    ...
    $q = "SELECT
        ...
        $q = $q."FROM `".TblModCatalog."`
              ...
              if($level_show === 'all') $q .=" AND `".TblModCatalog."`.`id` != '0'";
              else $q .=" AND `".TblModCatalog."`.`id` = '".$level_show."'";

              $q .=" GROUP BY `".TblModCatalog."`.`id`
...
//echo '<br>$arr='.$arr;print_r($arr);
   ...
   function show()
   {
         ...
         <tr>
          <td class="levels_tree">
              <div class="box-header with-border">
                  <div class="box-title">
                      <a href="<?=$_SERVER['PHP_SELF'].'?module='.$this->module.'&display='.$this->display.'&start='.$this->start.'&sort='.$this->sort.'&fltr='.$this->fltr.'&fln='.$this->fln;?>" title="<?=$this->multi['TXT_ROOT_CATEGORY'];?>"><img src="images/icons/categ.png" border="0" alt="<?=$this->multi['TXT_CATALOG_STRUCTURE'];?>" title="<?=$this->multi['TXT_CATALOG_STRUCTURE'];?>" /><?=$this->multi['TXT_CATALOG_STRUCTURE'];?></a>
                  </div>
                  <div class="box-body">
          ...
          <td valign="top">
           ...
           if ( $this->level>0 ) $this->ShowPathToLevel($this->level, NULL, $this->script );
               ...
               ?><a href="<?=$this->script?>&task=show_main_category"><i class="fa  fa-cog"></i> <?=$this->multi['TXT_SETTING_ROOT_CATALOG']?></a><?
           }
    ...
    // Date : 03.01.2014
    // Returns : true,false / Void
    // Description : Setting the main category
    // Programmer : Bogdan Iglinsky
    // ================================================================================================
     function showMainCategory(){
         ?>
         <div style="margin: 10px 0">
         <a CLASS="toolbar" href="<?=$this->script."&task=show&level=0"?>" onmouseout="MM_swapImgRestore();"
            onmouseover="MM_swapImage('restore','','images/icons/restore_f2.png',1);">
             <IMG src='images/icons/restore.png' alt="Go to:" align="middle" border="0"
                  name="restore">&nbsp;<?=$this->multi['TXT_OFFICE_DIRECTORY']?></a>
         </div>
         <?
         $this->ShowContentHTML(0);
     }
   ...
   // ================================================================================================
   function ShowContentHTML ($level_show = 'all'){
//       echo '$level_show='.$level_show;
       $arr_rows = $this->GetContent('limit',$level_show);
       $rows = count($arr_rows);
       ...
       <table border="0" cellpadding="0" cellspacing="1" class="table table-striped table-hover table-bordered">
        <tr>
         ...
         <Th class="THead"><?=$this->multi['FLD_DISPLAY']?></Th>
            <?if ( isset($this->settings['info_cat']) AND $this->settings['info_cat']=='1' ) {?>
         ...
         for( $i = 0; $i < $rows; $i++ ){
          ...
          <tr class="<?//=$class;?>">
           <td>
               ...
               <br/><?=$this->Form->Link( $this->script."&task=edit&id=".$row['id'], stripslashes( $row['id'] ), $editData);?>
           </td>
           <td><?=$this->Form->Link( $this->script."&task=edit&id=".$row['id'], stripslashes($row['prodname']), $editData);?></td>
...
//              echo $mas['cod_pli'];
              ...
              $this->Form->TextBox('cod_pli', $cod_pli, 10);
          }
      }
    ...
    */?>
    <tr>
     ...
     <td><b>
      ...
      $arr_categs = $this->GetCatalogInArray(NULL, NULL, NULL, NULL, 0, 'back');

      //print_r($arr_categs);
      //echo '<br>$this->level='.$this->level;
      $this->Form->Select( $arr_categs, 'new_id_cat', 'categ='.$this->level, NULL, NULL, 'categ='.$this->id );
      ?>
     </td>
    </tr>
    ...
    <tr>
     <td colspan="2">
      ...
      while( $el = each( $ln_arr ) )
      {
         ...
         if( $this->id!==NULL ){?>&nbsp;<?$this->Form->ButtonSimple("btn", $this->multi['TXT_EDIT'], NULL, "id='button".$lang_id."' onClick=\"EditTranslit('translit".$lang_id."','button".$lang_id."');\"");}
       ...
       <tr>
        <td>
         ...
         if ( !empty($mas['img_cat']) ) {
          ...
          $img_src = ImageK::getResizedImg($this->settings['img_path'].'/categories/'.$mas['img_cat'], "size_auto=200", 80, NULL);
          ?><img src="<?=$img_src;?>" border=0 alt="<?=  htmlspecialchars($this->treeCatData[$mas['id']]['name']);?>" title="" />
         <?
         }
         ?>
        </td>
    ...
    // ================================================================================================
     function controlKey($id){
         ...
         if( !empty($this->id) ){
             ...
             $this->Form->WritePreviewPanelNewWindow( "http://".NAME_SERVER.$CatalogLayout->Link($id) );
         }
   ...
   function CheckFields()
   {
    ...
    function down_relat_categ($table)
    {
     ...
     function ClearCategoryCache (){
         ...
         while( $el = each( $ln_arr ) ){
             $lang_id = $el['key'];
             Cache::instance()->delete('Catalog.Categories.treeCatData'.$lang_id);
             Cache::instance()->delete('Catalog.Categories.treeCatLevels'.$lang_id);
             Cache::instance()->delete('Catalog.Categories.treeCatList'.$lang_id);
             Cache::instance()->delete('Catalog.Categories.treeCatData.backend'.$lang_id);
             Cache::instance()->delete('Catalog.Categories.treeCatLevels.backend'.$lang_id);
             Cache::instance()->delete('Catalog.Categories.treeCatList.backend'.$lang_id);
         }
     }
