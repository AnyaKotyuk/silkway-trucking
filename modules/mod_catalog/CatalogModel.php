...
//       $str[0]=" ";
    ...
    function generateIdPropArra(){
        $array[0] = $this->GetListPositionsSortByDateCount($this->id_cat, 'nolimit', false);
//        var_dump($this->arr_current_img_params_value);
        if(is_array($this->arr_current_img_params_value)){
            foreach($this->arr_current_img_params_value as $key=>$value) {
                $array[$key] = $this->GetListPositionsSortByDateCount($this->id_cat, 'nolimit', false,$key);
            }
        }

        $array[-1] = $this->GetListPositionsSortByDateCount($this->id_cat, 'nolimit', false,-1);//price

        $array[-2] = $this->GetListPositionsSortByDateCount($this->id_cat, 'nolimit', false,-2);//brand
...
//        echo '$this->asc_desc='.$this->asc_desc;
        if(($paramId != 'sore' && $this->asc_desc=='asc') || ($paramId == 'sore' && $this->asc_desc=='desc')){
                    ...
                    if(!empty($paramQueryBody)){
                        ...
                        if($rows_prop>0){
                            for($j=0;$j<$rows_prop;$j++){
                                ...
                                else $arr_prop_lock[$row['id_prop']]++;
                            }
                        }
                    }
                }
...
//                echo '<br>$id='.$row['id'].' $price='.$row['price'];
                if($row['price']<$this->from || $row['price']>$this->to){
                    continue;
                }else{
                    $arr[] = $row;
                }
...
//        var_dump($arr);
    ...
    // Programmer : Ihor Trokhymchuk
    // ================================================================================================
    function GetListPositionsSortByDate($level=0, $limit='limit',  $sort = NULL, $asc_desc = "asc", $show_sublevels=false, $idParam = NULL,$strCat = '',$str_prop = '')
    {
        $this->paramsRowInit();
...
//                    $my_id_prop = implode(',',$keys_arr_prop_lock);
        ...
        for($i=0; $i<$rows; $i++){
            ...
            $price = $this->Currency->Converting($row['price_currency'], $curr_id, $row['price'], 2);
//            echo '<br>$price='.$price.' id='.$row['id'];
            $price_for_filter = $price*1000;
            ...
            $row['price'] = $price; //$this->Currency->ShowPrice($price);
...
//            var_dump($arrRow);
            ...
            for($i=0; $i<$rows; $i++){
                ...
                if(!isset($offset[$price])) $offset[$price] = 0;
...
//        var_dump($arr);
        ...
        for($i=0; $i<$rows; $i++){
            ...
            if(!empty($row['first_img'])){
                $row['image'] = $this->ShowCurrentImage($row['first_img'],'size_auto=215',85,NULL,NULL,$row['id'],true);
                ...
                $row['image'] = $this->ShowCurrentImage('/images/design/no-image.png','size_auto=215',85,NULL,NULL,$row['id'],true);
    ...
    // Change Request Nbr:
    // ================================================================================================
    function Link($id_cat = NULL, $id_prop = NULL, $param = NULL, $id_file = NULL, $lang_id=NULL )
    {
        if( !defined("_LINK")) {
        ...
        $arr_categs = $this->GetTopLevelsTranslit($id_cat, $lang_id);
        //echo '<br>$lang_id='.$lang_id.' $arr_categs=';print_r($arr_categs);
        $link=NULL;
        $translit_str = NULL;
        if(is_array($arr_categs)){
        ...
        if( !empty($id_prop) ) {
            ...
            if(is_numeric($id_prop))$translit_prop = $this->GetTranslitById($id_cat, $id_prop, $lang_id);
        ...
        switch($param){
            case 'goto':
                $link = $prefLink.'goto/'.$id_cat.'/'.$id_prop;
                break;
            case 'show_files':
                $link = NULL;
                if( !empty($id_file) ){
                    if( !empty($this->Logon->user_id) AND !empty($id_file) ){
                        ...
                        $link = $prefLink.'catalog/'.$translit_str.$translit_prop.'/files/'.$id_file;
                        //$link = $prefLink.Catalog_Upload_Files_Path.'/'.$id_prop.'/'.$tmp['path'];
                    }
                    else {
                        $link = $prefLink.'catalog/'.$translit_str.$translit_prop.'/files/'.$id_file;
                    }
                ...
                break;
            case 'print':
                $link = $prefLink.'print-it/catalog/'.$id_cat.'/'.$id_prop.'.html';
                break;
            default:
                if(CATALOG_TRASLIT){
                    if(empty($link)){
                        $link = $prefLink.'catalog/';
                    }else{
                        $link = $prefLink.$link;
                    }
                }else{
                    $link = $prefLink.'catalog/'.$link;
                }
        }
    ...
    function getMultiUrls(){
        ...
        foreach($mas_lang as $k=>$v){
            if($v['cod']==$this->lang_id){
                ...
                $CatalogNew = new CatalogLayout();
                $multiUrls[$SysLang->GetLangShortName($v['cod'])] = $CatalogNew->Link($this->id_cat, $this->id, NULL, NULL, $v['cod']);
            }
        }
        return $multiUrls;
    }
