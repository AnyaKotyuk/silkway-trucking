...
class Gallery {
    ...
    // Programmer : Yaroslav Gyryn
    // ================================================================================================
    function Link( $cat , $id = NULL, $getTranslit = false, $lang_id=NULL)
    {
        if( !defined("_LINK")) {
        ...
        if($cat!=NULL and $id==NULL){
            if($getTranslit){
                return $prefLink.'gallery/'.$this->Spr->GetTranslitByCod(TblModGalleryCat,$cat, $lang_id ).'/';
            }else{
                return $prefLink.'gallery/'.$cat.'/';
            }
        }elseif($id!=NULL){
            if($getTranslit){
                return $prefLink.'gallery/'.$this->Spr->GetTranslitByCod(TblModGalleryCat,$cat, $lang_id ).'/'.
                    $this->Spr->GetTranslitByCod(TblModGalleryTxt,$id, $lang_id ).'.html';
            }else{
                return $prefLink.'gallery/'.$cat.'/'.$id.'.html';
            }
        }else
            return $prefLink.'gallery/';
    } // end of function Link()

    /**
     * Gallery::makeMultiUrls()
    ...
    function getMultiUrls(){
        ...
        foreach($mas_lang as $k=>$v){
            if($v['cod']==$this->lang_id){
                $multiUrls[$SysLang->GetLangShortName($v['cod'])] = $this->Link($this->cat, $this->id);
            }else{
                //создавать нужно именно новый обьект, чтобы сформировать в нем новые массивы с данными на нужной языковой версии.
                $Gallery = new Gallery();
                $multiUrls[$SysLang->GetLangShortName($v['cod'])] = $Gallery->Link($this->cat, true, $this->id, $v['cod']);
                unset($News);
            }
        }
        //var_dump($multiUrls);
...
// ================================================================================================
 ...
 function QuickSearch($search_keywords){
   ...
   $filter_cr = ' OR ';
    ...
    $str_like = $this->build_str_like_for_full(TblModGalleryTxt.'.name', $search_keywords);
    $str_like .= $filter_cr.$this->build_str_like_for_full(TblModGalleryTxt.'.full', $search_keywords);
    $sel_table = "`".TblModGallery."`, `".TblModGalleryTxt."`, `".TblModGalleryCat."` ";
     ...
     $q ="SELECT
        ...
        (
            MATCH(`" . TblModGalleryTxt . "`.`name`) AGAINST('*".$search_keywords."*' IN BOOLEAN MODE) * 10 +
            MATCH(`" . TblModGalleryTxt . "`.`full`) AGAINST('*".$search_keywords."*' IN BOOLEAN MODE)
        ) AS `relev`
        FROM ".$sel_table."
        WHERE (".$str_like.")
        AND `" . TblModGallery . "`.id = `" . TblModGalleryTxt . "`.cod
        ...
        AND `" . TblModGallery . "`.dttm <= now()
        ORDER BY  `".TblModGallery."`.dttm DESC";
