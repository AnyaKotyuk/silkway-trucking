...
if(!isset($_REQUEST['avatar'])) $avatar=NULL;
else $avatar=$_REQUEST['avatar'];
...
$User->old_email = strip_tags(trim($old_email));
...
switch( $task ) {
    ...
    case 'save':
        ...
        if ( $User->SaveUser() ){
            ...
            if ($User->id==NULL) {
                ...
                $res = $User->update_user($User->GetSysUserIdByUserLogin($User->login), $User->group_id, $User->email, $User->enrol_date, $User->login_multi_use);
            ...
            if(!empty($err)){
               echo "<script>$.fancybox('".$err."');</script>";
            }
            if( $action=='return' ){
               $User->edit();
            }else{
   ...
   /*-------------- Search of users end -----------------*/
    ...
    case 'login_checkup':
        ...
        if( empty($User->email) ){
            ?><span style="font-size:10px; color:red;"><?=$User->Msg->show_text('_EMPTY_LOGIN_FIELD');?></span><?
            return false;
        }
        ...
        if( $User->old_login!=$User->login OR $check_up ){
            if( !$User->unique_login($User->login) ) {
                ?><span style="font-size:10px; color:red;"><?=$User->Msg->show_text('MSG_LOGIN_EXIST');?></span><?
            }
            else {?><span style="font-size:10px; color:green;"><?=$User->Msg->show_text('MSG_LOGIN_FREE');?></span><?}
        }
        break;
    ...
    case 'export_users_email':
        $User->ExportUsersEmail();
        break;
    case 'export_users_to_csv':
        $User->ExportUsersToCSV();
        break;
}
