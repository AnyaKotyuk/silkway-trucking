...
$FeedbackLayout->FrontendPages = check_init('FrontendPages', 'FrontendPages');

if (empty($FeedbackLayout->Dealers)) $FeedbackLayout->Dealers = check_init('DealerLayout', 'DealerLayout');
...
if(!isset($_REQUEST['surname'])) $FeedbackLayout->surname = NULL;
else $FeedbackLayout->surname = $FeedbackLayout->Form->GetRequestTxtData($_REQUEST['surname'], 1);
...
if(!is_ajax){
//    ?><!--<div class="contacts-name-form">--><?//=$FeedbackLayout->multi['_TXT_FORM_NAME'];?><!--</div>--><?//
}
//echo '$FeedbackLayout->task='.$FeedbackLayout->task;
  ...
  case 'send':
    ...
    if($FeedbackLayout->is_files==1){
        ...
        if ( $filename!=null and $_FILES["filename"]["error"]==0) {
          ...
          $FeedbackLayout->uploaddir = SITE_PATH.$FeedbackLayout->set_files_path;
    ...
    break;
  default:
      $FeedbackLayout->Dealers->ShowAllDealers();
    $FeedbackLayout->show_form();

    $loc = $FeedbackLayout->FrontendPages->getSpecContentByCod(3, true);
    $FeedbackLayout->show_map($loc);
