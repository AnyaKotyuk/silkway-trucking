...
class Feedback {
    ...
    */
    ...
    /**
     * Class method GetNewContacts
     * get new contacts
     * @return array for content
...
//        echo '<br>$q='.$q.' $res='.$res.' $this->db->result='.$this->db->result;
    ...
    /**
     * Class method UnsetNewStatus
     * get new contacts
     * @return array for content
     * @author Ihor Trokhymchuk <ihor@seotm.com>
     */
    function UnsetNewStatus($id){
        if(!$id){
            return false;
        }
        $q="UPDATE `".TblModfeedback."` SET
            `is_new`='0'
            WHERE
            `".TblModfeedback."`.`id`='".$id."'
           ";
        $res = $this->db->db_Query($q);
//        echo '<br>$q='.$q.' $res='.$res.' $this->db->result='.$this->db->result;
        if(!$res OR !$this->db->result){
