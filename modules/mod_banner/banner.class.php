...
class Banner {
    ...
    */
    ...
    /**
     * Class method GetTraslitByFileName
     *
     * @param name file, name dir
     * @author Bogdan Iglinsky  <bi@seotm.com>
     * @version 1.0 19.09.2013
     * @return name file
     */
    function GetTraslitByFileName($str = '',$uploaddir = ''){
        $arr = explode('.',$str);
        ...
        unset($arr[$count-1]);
        $str_arr = implode(',',$arr);
        $str_name = $this->Crypt->GetTranslitStr($str_arr);
        if(!is_file($uploaddir.$str_name.'.'.$exp)){
            $str_return = $str_name.'.'.$exp;
        }else{
            $str_return = $str_name.time().'.'.$exp;
        }
        return $str_return;
    }
