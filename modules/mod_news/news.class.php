...
//                  echo '<br/> $q='.$q.' $res='.$res;
    ...
    function loadTreeNewsCat(){
     ...
     // Programmer :  Yaroslav Gyryn
     // ================================================================================================
     function ConvertDate($date_to_convert, $showTimeOnly = true, $separator = ".", $showMonth = false){
        ...
        return $day.$separator.$month.$separator.$year." ".$time1;
    } // end of function ConvertDate()




    function GetDate($date_to_convert, $task){
        $tmp = explode("-", $date_to_convert);
        $tmp2 = explode(" ", $tmp[2]);
        $month = NULL;
        $day = NULL;
        ...
        switch($task) {
            case 'day':
                return $day;
            case 'month':
                return $months[$lang_id][$month];
        }
    } // end of function ConvertDate()



    function ConvertDateArchive($date_to_convert, $separator = ".", $showMonth = true){
        $tmp = explode("-", $date_to_convert);
...
//        echo '<br>$img_with_path='.$img_with_path;
    ...
    function GetNewsData( $news_id = NULL )
    {
        ...
        $q = "SELECT *
              FROM `".TblModNews."`, `".TblModNewsNames."`, `".TblModNewsFull."`, `".TblModNewsShort."`
...
//        echo '<br>'.$q.' <br/>$res='.$res.' $this->db->result='.$this->db->result;
    ...
    function ShowNewsByDate($day)

    {

        $c_date = date("Y-m-d", strtotime($day));

        //$q = "select * from `".TblModNews."` where 1 and `status`!='i' and `start_date` LIKE '%".$c_date."%' order by `start_date` desc";
        $q = "SELECT
            *
        FROM
      ...
      WHERE
        `".TblModNews."`.id = `".TblModNewsShort."`.id_news and
        `".TblModNews."`.id = `".TblModNewsNames."`.id_news and
        `".TblModNews."`.id_cat = `".TblModNewsCat."`.cod and
         `".TblModNewsCat."`.lang_id = '"._LANG_ID."' and
         `".TblModNews."`.status !='i' and
         `".TblModNews."`.start_date LIKE '%".$c_date."%'
        order by
         `".TblModNews."`.start_date desc
...
echo $q;
        ...
        //echo '<br>$q= '.$q.'<br/> $res= '.$res.'<br>$rows='.$rows;
...

        {
            ...
            $row = $this->db->db_FetchAssoc($res);
var_dump($row);
            if($row['start_date'] > DATE_FOR_LINK ) {

                $link = $this->Link($row['id_cat'] ,$row['link_cat']);

                $linkCat = $this->Link($row['id_cat']);

            }

            else {

                $link = 'news_'.$row['id'].'.html';

                $linkCat = 'newscat_'.$row['id_cat'].'html';
            ...
            <div class="one-item">
                <div class="short" style="">
                    ...
                    if(!empty($row['main_img'])){
                        ?><div class="alt" style="max-width:130px;"><?
                        echo $this->ShowCurrentImage('/images/mod_news/'.$row['main_img'], 'size_width=120', 85, 'txt', 'alt="'.$row['name'].'" title="'.$row['name'].'" style="padding: 0px 8px 5px 0px;" align="left"');
                        ?></div><?
                    }?>
                </div>
                <div class="date_line"><?=$this->ConvertDate($row['start_date']);?></div>
                <a class="news-name" href="<?=$link?>">
                    <?/*=stripslashes($row['sbj']);
                    if($row['adv']) $this->FrontForm->ShowAdvMarker();*/
                    //                    var_dump($row);
                    /*echo $this->ShowIcons($row);*/?><!--<br/>-->
                </a>
                <div class="short-text">
                    <?/* echo strip_tags(stripslashes($row['short']),"<a><b><strong><u><i><em>");*/?>
                </div>
            ...
            <div class="news_block">
                ...
                <a href="<?=$link;?>"><h1 style="text-decoration:none;"><?=$row['sbj'];?><?if($row['adv']) $this->FrontForm->ShowAdvMarker();?></h1></a>

            <span class="date_line">

                <a href="<?=$linkCat;?>" class="date_line" title="<?=$row['categ']?>"><strong><?=$row['categ']?></strong></a>

                &nbsp;/&nbsp;<?=$this->ConvertDate($row['start_date']);?>

            </span>

                <div class="short" style="line-height:14px;">

                    <?

                    if(!empty($row['img'])) {

                        $subject = htmlspecialchars(stripslashes($row['sbj']));

                        echo $this->ShowCurrentImage('/images/mod_news/'.$row['img'], 'size_auto=150', 85, 'txt', 'alt="'.$subject.'" title="'.$subject.'" style="padding: 0px 8px 5px 0px;" align="left"');

                    }

                    echo strip_tags(stripslashes( $row['short']),"<p><a><b><strong><u><i><em>")?> / <a href="<?=$link;?>" class="short" style="COLOR: #CC0000; FONT-WEIGHT: normal" title="Читать новость дальше">подробнее »»</a>
    ...
    } //end of function ShowNewsByDate
    ...
    // Programmer : Yaroslav Gyryn
    // ================================================================================================
    function Link( $cat = NULL, $str_news = NULL, $lang_id=NULL)
    {
...
//        var_dump($str_news);
        ...
        $link_return = $prefLink.'news/'.$link_return;

//        var_dump($link_return);
        return $link_return;
    ...
    function GetIdNewsByStrNews($str_news){
        $q = "SELECT `".TblModNews."`.`id`, `".TblModNewsNames."`.`lang_id`
...
//            echo '<br>'.$q.' $res='.$res.' $this->db->result='.$this->db->result;
   ...
   //echo $body;
    ...
    function SetSeoData($pagetxt)
    {
        ...
        $this->keywords = '';
        //var_dump($pagetxt);
        if(!empty($pagetxt['mtitle'])) {
            $defaultTitle = $pagetxt['mtitle'];
        }else{
            $defaultTitle = $pagetxt['pname'];
        }
        if(!empty($this->id_cat)){
            $defaultTitle = $this->treeNewsCat[$this->id_cat]['name'].' | '.$defaultTitle;
        }
        $defaultDescr = $pagetxt['mdescr'];
        if(!empty($this->id_cat)){
            $defaultDescr = $this->treeNewsCat[$this->id_cat]['name'].' | '.$defaultDescr;
        }
        $defaultKeywords = $pagetxt['mkeywords'];
        if(!empty($this->id_cat)){
            $defaultKeywords = $this->treeNewsCat[$this->id_cat]['name'].', '.$defaultKeywords;
        }
...
//            echo '<br>$rows='.$rows;
            if($rows>0){
                ...
                if(empty($this->description )){
                    $this->description = $row['name'].' | '.$defaultDescr;
                }
                $this->keywords = $row['keywords'];
                if(empty($this->keywords)){
                    $this->keywords = $row['name'].', '.$defaultKeywords;
                }
            }
            return;
            ...
            $this->h1 = $this->FrontendPages->GetH1();
        ...
        */
        ...
        if($this->page>1) $this->title .= ' | Сторінка '.$this->page;
    ...
    function GetIdCatByStr($str_cat = NULL){
        ...
        /*$q1 = "SELECT *
              FROM `".TblModNewsCat."`
             ";
        $res2 = $this->db->db_Query($q1);
        if(!$res2 OR !$this->db->result){
            return false;
        }
        $rows2 = $this->db->db_GetNumRows($res2);
        $array1 = array();
        $array2 = array();
        for($i=0;$i<$rows2;$i++){
            $row2 = $this->db->db_FetchAssoc();
            $array2[$i] = $row2;
            if($array2[$i]['level'] == $row['cod'])
                $array1[$i] = $array2[$i]['level'];
                //return $array2[$i]['level'];
                //return $row['cod'];
        }*/
        /*$keys = array_keys($this->treeNewsCat);
        $size = sizeof($keys);
...
//        echo '<br>q='.$q.' res='.$res.' $tmp_db->result='.$this->db->result;
    ...
    function GetNRowsNews($limit=false)
    {
    ...
    function GetNRowsLastNews($limit=false)
    {
        $q = "SELECT
            *
        FROM
        ...
        WHERE
         ...
         ";
        if( $this->fltr!='' ) $q = $q.$this->fltr;
        $q = $q." order by `".TblModNews."`.start_date DESC ";
        if($limit) $q .= " limit ".$this->start.",".$this->display."";
        $res = $this->db->db_Query( $q );
//        echo "<br>q=".$q." res=".$res;
        if( !$res OR !$this->db->result) return false;
    ...
    function GetNRowsTopMainNews($limit=false){
        $q = "SELECT
        `".TblModNews."`.*,
        `".TblModNewsShort."`.*,
        `".TblModNewsTop."`.`name` as `top_name`,
        `".TblModNewsTop."`.`short` as `top_short`,
        `".TblModNewsTop."`.`image` as `top_img`,
        `".TblModNewsNames."`.*
        FROM
            `".TblModNews."`
            LEFT JOIN `".TblModNewsTop."` ON (`".TblModNews."`.`id` = `".TblModNewsTop."`.`cod` AND `".TblModNewsTop."`.`lang_id`='".$this->lang_id."'),
            `".TblModNewsShort."`, `".TblModNewsNames."`
        WHERE
            `".TblModNews."`.id = `".TblModNewsShort."`.id_news and
            `".TblModNews."`.top = '1' and
            `".TblModNews."`.id = `".TblModNewsNames."`.id_news and
            ...
            `".TblModNews."`.`status`='a'
         ";
        $q = $q." AND `".TblModNews."`.`top_main`='1'";
        if( $this->fltr!='' ) $q = $q.$this->fltr;

        $q = $q." ORDER BY
                `".TblModNews."`.`top_main` DESC, `".TblModNews."`.`start_date`  DESC ";
        if($limit) $q .= " limit ".$this->start.",".$limit."";
    ...
    function GetNRowsTopNews($limit=false, $checkTopNews=false)
    {
        $q = "SELECT
        `".TblModNews."`.*,
        `".TblModNewsShort."`.*,
        ...
        WHERE
            ...
            `".TblModNews."`.`status`='a'
         ";
        if( $checkTopNews ) $q = $q." AND `".TblModNews."`.`top_main`!='1'";
        if( $this->fltr!='' ) $q = $q.$this->fltr;

        $q = $q." ORDER BY
                `".TblModNews."`.`top_main` DESC, `".TblModNews."`.`start_date`  DESC ";

        if($limit) $q .= " limit ".$this->startTop.",".$limit."";
    ...
    function GetArchiveNRows($day, $limit=false) {
        $c_date = date("Y-m-d", strtotime($day));
        $q = "SELECT
            *
        FROM
        ...
        WHERE
            ...
            `".TblModNewsNames."`.`name`!='' and
            `".TblModNews."`.`status`!='n' and
            `".TblModNews."`.start_date LIKE '%".$c_date."%' and
            `".TblModNews."`.`visible` = '1'
         ";
        if( $this->fltr!='' ) $q = $q.$this->fltr;
        $q = $q." order by `".TblModNews."`.display DESC ";
...
//        echo "<br>q=".$q." res=".$res;
    ...
    function GenerateVIPRSSNews()
    {
        ...
        $q = "SELECT
            ...
            `".TblModNewsNames."`.name,
            `".TblModNewsNames."`.link,
            `".TblModNewsShort."`.short,
            `".TblModNewsFull."`.full
        FROM
            `".TblModNews."`, `".TblModNewsNames."`,`".TblModNewsShort."`, `".TblModNewsFull."`
        WHERE
            `".TblModNews."`.status != 'e'
            AND
            `".TblModNews."`.to_rss = '1'
            ...
            AND
            `".TblModNews."`.id = `".TblModNewsShort."`.id_news
            AND
            `".TblModNewsShort."`.lang_id = '"._LANG_ID."'
            AND
            `".TblModNews."`.id = `".TblModNewsFull."`.id_news
            AND
            `".TblModNewsFull."`.lang_id = '"._LANG_ID."'
        ORDER BY
            `".TblModNews."`.id desc
  ";
     ...
     <channel>
        <title>Спортивний Житомир</title>
        <link>http://sportzt.com/</link>
        <language>ru</language>
        <description>Спортивний Житомир</description>
        <image>
            <url/>
            <title>Спортивний Житомир</title>
            <link>http://sportzt.com/</link>
...
//    var_dump($rows); die();
        for($i = 0; $i<$rows; $i++)
        {
            ...
            if(($row['to_rss'] == 0 || $row['to_rss'] == '0')){
                continue;
            }
            $sbj = strip_tags(stripslashes($row['name']));
            $full = strip_tags(stripslashes( $row['full']),'<p><img>');
            $short =strip_tags(stripslashes($row['short']/*$this->Spr->GetNameByCod( TblModNewsSprShrt, $row['id'] )*/ ));
            ...
            $data = $data.'
        <item>
            <title>'.$sbj./*strip_tags(htmlspecialchars($row['sbj']/* $this->Spr->GetNameByCod( TblModNewsSprSbj, $row['id'] )*/'</title>
            <link>http://sportzt.com'.$row['link_cat'].$row['link'].'.html</link>
            <description>'.$short.'</description>
            <category>'.$this->Spr->GetNameByCod( TblModNewsCat, $row['id_cat'] ).'</category>
            <pubDate>'.date("D, d M Y H:i:s", strtotime($row['start_date'])).' +0200'.'</pubDate>
            <enclosure url="" type="" length=""/>
            <yandex:full-text>'.$full.'</yandex:full-text>
        </item>';
        }
        $data = $data.'</channel></rss>';
        //echo $data;
        $f = $_SERVER['DOCUMENT_ROOT'].'/rss.xml';
        $hhh = fopen($f, "w");
        fwrite($hhh, $data);
        fclose($hhh);

    } //end of function GenerateVIPRSSNews
} //--- end of class News
