...
if( !isset( $_REQUEST['display'] ) OR ($task=='search') ) $display = 50;
else $display = $_REQUEST['display'];

if( !isset( $_REQUEST['start'] ) ) $start = 0;
else $start = $_REQUEST['start'];

if( !isset( $_REQUEST['sort'] ) ) $sort = 'start_date';
...
if( !isset( $_REQUEST['fltr_dt_from'] ) ) $fltr_dt_from=NULL;
else $fltr_dt_from = $_REQUEST['fltr_dt_from'];

if( !isset( $_REQUEST['fltr_dt_to'] ) ) $fltr_dt_to=NULL;
else $fltr_dt_to = $_REQUEST['fltr_dt_to'];
...
if( !isset( $_REQUEST['link'] ) ) $link = NULL;
else $link = $_REQUEST['link'];
...
if( !isset( $_REQUEST['toRss'] ) ) $toRss = 0;
else $toRss = 1;
...
if( !isset( $_REQUEST['status'] ) ) $status = 'i';
...
if( !isset( $_REQUEST['tags'] ) ) $tags = NULL;
else $tags = $_REQUEST['tags'];
...
if( !isset( $_REQUEST['dispatch_sbj'] ) ) $dispatch_sbj = $News->multi['TXT_DISPATCH_TITLE_DEFAULT'];
...
$News->sort = $sort;
$News->srch = $srch;
$News->fltr_dt_from = $fltr_dt_from;
$News->fltr_dt_to = $fltr_dt_to;
$News->fltr = $fltr;
$News->fltr2 = $fltr2;
$News->img = $img;
...
$News->line = $line;
$News->link = $link;
$News->translit_old = $translit_old;
$News->translit = $translit;
$News->name_ = $name;
$News->h1_ = $h1;
$News->main = $main;
$News->short_ = $short;
...
switch( $task ) {
    ...
    case 'save':
        ...
        $res = $News->save();
//        var_dump($res);
//        echo '<br>$res='.$res.' $News->id='.$News->id;


        if ($res)
        {
            $News->GenerateVIPRSSNews();
