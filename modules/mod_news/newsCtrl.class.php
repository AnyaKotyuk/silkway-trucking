...
class NewsCtrl extends News{
    ...
    var $sel = NULL;
    var $is_tags = NULL;
    public $drupal_id = NULL;
    public $cnt_found_rows_all = NULL;
    ...
    */
    ...
    /**
     * Class method getNews
     * function for get all data for news
     * @author Igor Trokhymchuk  <ihor@seotm.com>
     * @version 1.1, 12.12.2015
     * @return true or false
     */
    function getNews($limit=false, $getNewsCount=false){
        if( !$this->sort ) $this->sort='display';
        //if( strstr( $this->sort, 'display' ) )$this->sort = $this->sort.' desc';
        if($getNewsCount){
            $q = "SELECT COUNT(`".TblModNews."`.`id`) as `cnt`";
        }else {
            $q = "SELECT
                    `" . TblModNews . "`.*,
                    `" . TblModNewsNames . "`.`name`,
                    `" . TblModNewsShort . "`.`short`,
                    `" . TblModNewsShort . "`.`is_full`
                 ";
        }
        $q .= "
              FROM `".TblModNews."`
              LEFT JOIN `".TblModNewsNames."` ON (`".TblModNews."`.`id`=`".TblModNewsNames."`.`id_news` AND `".TblModNewsNames."`.`lang_id`='".$this->lang_id."')
              LEFT JOIN `".TblModNewsShort."` ON (`".TblModNews."`.`id`=`".TblModNewsShort."`.`id_news` AND `".TblModNewsShort."`.`lang_id`='".$this->lang_id."')
              WHERE 1 ";
        if( $this->srch ){
            $q .= " AND `".TblModNewsNames."`.`name` LIKE '%".$this->srch."%' ";
        }
        if( $this->fltr ){
            $q .= " AND `".TblModNews."`.`id_cat` = '".$this->fltr."' ";
        }
        if( $this->fltr2 ){
            $q .= " AND `".TblModNews."`.`status`='".$this->fltr2."'";
        }
        if( $this->fltr_dt_from ){
            $q .= " AND `".TblModNews."`.`start_date`>='".$this->fltr_dt_from."'";
        }
        if( $this->fltr_dt_to ){
            $q .= " AND `".TblModNews."`.`start_date`<='".$this->fltr_dt_to."'";
        }
        $q .= " ORDER BY `".TblModNews."`.`".$this->sort."` desc";
        if($limit){
        ...
        $rows_all = $this->Right->db_GetNumRows();
        //echo '<br>$rows_all='.$rows_all;
        $arr=array();
    ...
    /**
     * Class method getNewsCount
     * function for get count of news
     * @author Igor Trokhymchuk  <ihor@seotm.com>
     * @version 1.1, 12.12.2015
     * @return true or false
     */
    function getNewsCount($limit=false){
        return $this->getNews($limit, true);
    }
    ...
    function showSearchPanel(){
        ...
        <tr>
            <td colspan="14">
                <h3><?php echo $this->multi['TXT_SEARCH_PANEL'];?></h3>
                <table>
                    <tr class="TR1">
                        <td><?php echo $this->multi['_FLD_NEWS_SUBJECT'];?>:</td>
                        <td><?$this->Form->Textbox('srch', $this->srch, 50, 'style="width:200px;"');?></td>
                    </tr>
                    <tr class="TR2">
                        <td><?php echo $this->multi['_FLD_NEWS_CATEGORY'];?>:</td>
                        <td>
                            <?
                            $arr_cat =  $this->Spr->GetStructureInArrayForNews(TblModNewsCat, 0, $this->lang_id, $this->multi['TXT_NEWS_ALL_CATEGORIES'], '&nbsp;&nbsp;', 1, 1, 1, 'back', NULL);
                            $this->Form->Select( $arr_cat, 'fltr', $this->fltr, 50, 'style="width:200px;"' );
                            ?>
                        </td>
                    </tr>
                    <tr class="TR1">
                        <td><?php echo $this->multi['_FLD_NEWS_STATUS'];?>:</td>
                        <td >
                            <?
                            ...
                            $this->Form->Select( $arrStatus, 'fltr2', $this->fltr2, 50, 'style="width:200px;"' );
                            ?>
                        </td>
                    </tr>
                    <tr class="TR2">
                        <td><?php echo $this->multi['_FLD_NEWS_STARTDATE'];?>:</td>
                        <td>
                            <?
                            echo $this->multi['FLD_STAT_DATE_FROM'].' ';
                            if( $this->fltr_dt_from!=NULL ) $start_date_val=$this->fltr_dt_from;
                            else $start_date_val=strftime('%Y-%m-%d %H:%M', strtotime('-1826 days'));
                            ...
                            echo $this->multi['FLD_STAT_DATE_TO'].' ';
                            if( $this->fltr_dt_to!=NULL ) $start_date_val=$this->fltr_dt_to;
                    ...
                    <tr class="TR1">
                        ...
                        <td>
                            <?
                            //$this->Form->Hidden('task', 'search');
                            $onClick = "$('#task').val('search');$('#".$this->Form->name."').submit();";
                            $this->Form->ButtonSimple('search', $this->multi['TXT_SEARCH'], 20, 'onClick="'.$onClick.'"');
                            if($this->cnt_found_rows_all!=NULL AND $this->task=='search'){
                                echo ' '.$this->multi['TXT_NEWS_SEARCH_RESULT'].': '.$this->cnt_found_rows_all;
                            }
                            ?>
                        </td>
                    </tr>
    ...
    */
    function show(){
        $row_arr = $this->getNews(true);
        $rows = count($row_arr);
        //кол-во для постраничности и отображения в результатах поиска
        $this->cnt_found_rows_all = $this->getNewsCount(false);
        $frm = new Form('fltr');
        ...
        <tr>
            ...
            <!--<th class="THead"><?//=$this->multi['TXT_NEWS_TO_RSS']?></Th>-->
     ...
     for( $i = 0; $i <$n; $i++ )
     {
        ...
        $this->Form->Link( $script."&task=show&fltr=id_category=".$row['id_cat'], $row['Name_cat'], $this->multi['_TXT_FLT_DATA'] );
...
*/
       ...
       $a=$a-1;
        ...
        //echo '<br>$rows='.$rows;
        $this->Form->WriteLinkPages( $script1, $this->cnt_found_rows_all, $this->display, $this->start, $this->sort );
...
<!--         <td width="120"><b>--><?//=$this->multi['_FLD_NEWS_CATEGORY']?><!--</b></td>-->
         <td>
          ...
          if($id!=NULL ) $this->Err!=NULL ? $toRss = $this->toRss : $toRss=$mas['to_rss'];
          else $toRss=1;
          if($toRss==1) $checked='checked';
          else $checked='';
          // ТОП
          ?>
<!--          &nbsp;&nbsp;&nbsp;<input type="checkbox" name="toRss" align="left" --><?//=$checked;?><!--  <b>--><?//=$this->multi['TXT_NEWS_TO_RSS']?>
 ...
 <tr>
  <td>
    ...
    while( $el = each( $ln_arr ) ){
        ...
        $this->Form->SpecialTextArea( $this->Form->textarea_editor, 'short['.$lang_id.']', stripslashes($val), 10, 80, 'class="contentInput"', $lang_id, 'short' );
        echo "\n <br>";
        }
        if( isset($this->settings['full_descr']) AND $this->settings['full_descr']==1){
        echo "\n <tr>";
...
//    }
    ...
    */
    ...
    //echo 'epic fail='.$this->drupal_id;
        //$q = "SELECT * FROM ".TblModNews." WHERE `drupal_id`='".$this->drupal_id."'";
        ...
        if( $rows>0 )   //--- update
        {
            ...
            if(isset($this->drupal_id) AND !empty($this->drupal_id)){
                $q .= ", `drupal_id`='".$this->drupal_id."'";
            }
         ...
         else          //--- insert
         {
            ...
            $q = "INSERT INTO `".TblModNews."` SET
             ...
             if( isset($this->settings['dt']) AND $this->settings['dt']==1){
                 $q.=",`start_date` = '".$this->start_date.":00'
    ...
    //    echo "<br / > res saveMainImg = ".$res;
        ...
        $q_cat = "SELECT
                ...
                `".TblModNewsCat."`.name,
                `".TblModNewsCat."`.cod,
                `".TblModNewsCat."`.lang_id
            FROM
                `".TblModNews."`, `".TblModNewsCat."`
            WHERE
                `".TblModNews."`.id_cat = `".TblModNewsCat."`.cod
                AND `".TblModNewsCat."`.lang_id='".$this->lang_id."'
                AND `".TblModNews."`.`id`='".$this->id."'
                ";
        $res_cat = $this->db->db_Query($q_cat);
        $row_cat = $this->db->db_FetchAssoc($res_cat);
        $link = $this->Link($row_cat['cod']);
        $q1 = "UPDATE `".TblModNews."` SET
        ...
        /*
        $res_cat = $this->db->db_Query($q_cat);
        //echo "<br> q = ".$q." $res_cat = ".$res_cat;
        $rows_cat = $this->db->db_GetNumRows($res_cat);
        $arr_cat = array();
        for( $i=0; $i<$rows_cat; $i++ ) {
            $arr_cat[] = $this->db->db_FetchAssoc($res_cat);
        }
        for( $i=0; $i<$rows_cat; $i++ ) {
            $row_cat = $arr_cat[$i];
            $link = $this->Link($row_cat['cod']);
            $q1 = "UPDATE `".TblModNews."` SET Name_cat = '".$row_cat['name']."' WHERE id_cat = '".$row_cat['cod']."' ";
            $res1 = $this->db->db_Query($q1);
            $q2 = "UPDATE `".TblModNews."` SET link_cat = '".$link."' WHERE id_cat = '".$row_cat['cod']."' ";
