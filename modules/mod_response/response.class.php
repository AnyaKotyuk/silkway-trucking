 ...
 class Response {
   ...
   function edit()
   {
    ...
    while( $el = each( $ln_arr ) )
    {
         ...
         echo "\n <tr>";
         echo "\n <td><b>".$this->multi['AUTOR'].":</b>";
         echo "\n <td>";
         if (isset($txt[$lang_id]['autor']))
            $autor = $txt[$lang_id]['autor'];
         else $autor='';
         if( $this->id!=NULL ) $this->Err!=NULL ? $val=$this->autor[$lang_id] : $val=$autor;
         else $val=$this->autor[$lang_id];
         $this->Form->TextBox( 'autor['.$lang_id.']', stripslashes($val), 80 );
...
//        echo '<br>'.$q.' <br/>$res='.$res.' $this->Rights->result='.$this->Rights->result;
    ...
    if ( empty($this->id) ){
     ...
     while( $el = each( $ln_arr ) )
     {
       ...
       if(isset($this->autor[ $el['key'] ])) $autor = addslashes( trim($this->autor[ $el['key'] ]) );
       else $autor= NULL;
