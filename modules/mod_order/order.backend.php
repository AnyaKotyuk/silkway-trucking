...
if (!isset($_REQUEST['fltr2']))
    ...
    $my->fltr2 = $_REQUEST['fltr2'];

if (!isset($_REQUEST['fltrUserId']))
    $my->fltrUserId = NULL;
else
    $my->fltrUserId = $_REQUEST['fltrUserId'];
...
if (!isset($_REQUEST['ttn']))
    ...
    $my->ttn = $my->Form->GetRequestTxtData($_REQUEST['ttn'], 1);
...
if (!isset($_REQUEST['old_status']))
    $my->old_status = NULL;
else
    $my->old_status = $_REQUEST['old_status'];
...
$my->script = $_SERVER['PHP_SELF'] . "?module=$my->module&display=$my->display&start=$my->start&sort=$my->sort&fltr=$my->fltr&fltr2=$my->fltr2&fltrUserId=$my->fltrUserId&id_cat=" . $my->id_cat . '&property=' . $my->property . '&srch=' . $my->srch;
...
switch ($my->task) {
    ...
    case 'load_prod_data':
        $my->LoadProdData();
        break;
    case 'exportOrderListId':
        $my->ExportOrdersToCSV();
        break;
}
?>